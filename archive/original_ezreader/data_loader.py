"""
Data loader for the Provo Corpus.

Parses the Provo eye-tracking CSV into sentence-level batches
suitable for training the Neural EZ Reader.

Key output per word:
    - word (str)
    - word_length (int)
    - predictability (float, 0-1, OrthographicMatch from cloze norms)
    - first_fixation_duration (float, ms) - 0 if skipped, NA if missing
    - gaze_duration (float, ms) - IA_FIRST_RUN_DWELL_TIME
    - total_reading_time (float, ms) - IA_DWELL_TIME
    - was_skipped (bool)
    - regression_in (bool)
"""
...
def load_provo(
    ...
    min_sentence_length : int
        Skip sentences shorter than this.
    max_sentence_length : int
        Skip sentences longer than this.
    participants : list[str] or None
        If provided, only load data for these participant IDs.
    ...
    # Key = (participant_id, text_id, sentence_number)
...
def get_unique_sentences(dataset: List[SentenceData]) -> Dict[Tuple[int, int], List[str]]:
    ...
    Useful for getting the unique sentence set (independent of participant).
    """
    sentences = {}
    for sd in dataset:
...
def compute_word_frequency_from_corpus(dataset: List[SentenceData]) -> Dict[str, int]:
    ...
    (Not a real frequency list, but useful as a fallback.)
    """
...
class AggregatedSentence:
    ...
    This is the clean training target (no single-participant noise).
    """
    ...
    def __init__(self, text_id: int, sentence_number: int,
        ...
        self.mean_trt = mean_trt          # mean total reading time per word (ms)
        self.mean_ffd = mean_ffd          # mean first fixation duration per word (ms)
        self.mean_gaze = mean_gaze        # mean gaze duration per word (ms)
        self.skip_rate = skip_rate        # proportion of participants who skipped (0-1)
...
def aggregate_by_sentence(
    ...
    For each unique sentence, averages reading times across all participants.
    This produces much cleaner training targets.

    Parameters
    ----------
    dataset : list[SentenceData]
        The full per-participant dataset from load_provo().
    min_participants : int
        Only include sentences with at least this many participant observations.
    ...
    for (text_id, sent_num), observations in groups.items():
        ...
        # Use the first observation for tokens/predictabilities (same for all participants)
        ref = observations[0]
        n_words = len(ref.words)

        # Check all observations have the same number of words
        ...
        # Accumulate per-word stats across participants
        trt_sums = [0.0] * n_words
        ffd_sums = [0.0] * n_words
        gaze_sums = [0.0] * n_words
        skip_counts = [0] * n_words
        trt_counts = [0] * n_words  # count non-zero observations
        ...
        # Compute means (use 0 if no non-skip observations)
...
def split_aggregated(
    ...
    """Split aggregated sentences by TEXT to avoid data leakage."""
    import random
...
def split_dataset(
    ...
    All observations from the same text go into the same split.
    """
    import random
    rng = random.Random(seed)
...
if __name__ == "__main__":
    ...
    print(f"\nSplits (by text): train={len(train):,}, val={len(val):,}, test={len(test):,}")
