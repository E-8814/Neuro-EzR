...
class Simulation(object):
    ...
    def __init__(self, sentence, realtime=False, noise=False, initial_time=0, initial_fixation=1, trace=True):
        ...
        self.__saccade = None
        self._repeated_attention = 0 # time on repeated attention due to integration failure
    ...
    def _prepare_saccade(self, new_fixation_point, word, canbeinterrupted=True):
        ...
        if self.__canbeinterrupted:
            ...
            if (float(self.fixation_point) <  float(new_fixation_point) - len(word)/2) or (float(self.fixation_point) >  float(new_fixation_point) + len(word)/2): # start a saccade only if fixation away from the current word
                self.__saccade = self.env.process(self._saccadic_programming(new_fixation_point=new_fixation_point, word=word, canbeinterrupted=canbeinterrupted))
    ...
    def _saccadic_programming(self, new_fixation_point, word, regression=False, canbeinterrupted=True):
        """
        ...
        """
        self._collect_action(Action('Started saccade', " ".join(['Planned saccade:', str(self.fixation_point), '->',  str(new_fixation_point), 'Word:', word]), self.time))
        ...
        try:
            ...
            self._collect_action(Action('Interrupted saccade programming', " ".join(['Planned saccade:', str(self.fixation_point), '->',  str(new_fixation_point), 'Word:', word]), self.time))
            self.__canbeinterrupted = True
            ...
            self._collect_action(Action('Saccade programming finished', " ".join(['Planned saccade:', str(self.fixation_point), '->',  str(new_fixation_point), 'Word:', word]), self.time))

            tM2 = self.model_parameters['saccade_finishing'] #tM2

            yield self._timeout(tM2)

            self._collect_action(Action('Saccade finished', " ".join(['Planned saccade:', str(self.fixation_point), '->',  str(new_fixation_point), 'Word:', word]), self.time))
            ...
            if self.__plan_sacade:
                self.__saccade = self.env.process(self._saccadic_programming(new_fixation_point=self.__plan_sacade[0], word=self.__plan_sacade[1], canbeinterrupted=self.__plan_sacade[2]))
                self.__plan_sacade = False
            else:
    ...
    def _integration(self, last_letter, new_fixation_point, new_fixation_point2, elem, elem_for_attention, next_elem):
        """
        ...
        """
        self._collect_action(Action('Started integration', " ".join(["Word:", str(elem.token)]), self.time))
        ...
        if float(elem.integration_failure) >= random_draw:
            ...
            self._collect_action(Action('Failed integration', " ".join(["Word:", str(elem.token)]), self.time))

            # if failed integration, start saccade back to that word and attend the word again
            self._prepare_saccade(new_fixation_point, str(elem_for_attention.token), canbeinterrupted=False)
            self.env.process(self._attend_again(last_letter, new_fixation_point2, elem=elem_for_attention, next_elem=next_elem))

        else:

            self._collect_action(Action('Successful integration', " ".join(["Word:", str(elem.token)]), self.time))
    ...
    def _attend_again(self, last_letter, new_fixation_point, elem, next_elem):
        ...
        if self.attended_word != old_attended_word:
            ...
            self._collect_action(Action('Attention shift', " ".join(["To word:", str(elem.token)]), self.time))
        ...
        if elem.token == "None":
            ...
            self._repeated_attention += time_familiarity_check
            yield self._timeout(time_familiarity_check)

            self._collect_action(Action('L1 FAKE', " ".join(["Word:", str(self.attended_word.token)]), self.time))
        ...
        # otherwise proceed with the standard attention
            ...
            self._collect_action(Action('L1', " ".join(["Word:", str(self.attended_word.token)]), self.time))
            ...
            self._repeated_attention += time_lexical_access

            yield self._timeout(time_lexical_access)

            self._collect_action(Action('L2', " ".join(["Word:", str(self.attended_word.token)]), self.time))
    ...
    def visual_processing(self, sentence):
        ...
        for i, elem in enumerate(sentence):
            ...
            yield self._timeout(self._repeated_attention) # add extra time if there is a repeated processing of some previous word
            ...
            if float(self.model_parameters["probability_correct_regression"]) >= random_draw:
                self.env.process(self._integration(last_letter=first_letter+len(elem.token), new_fixation_point=first_letter - 0.5 + len(elem.token)/2, new_fixation_point2=new_fixation_point, elem=elem, elem_for_attention=elem, next_elem=next_elem))
            else:
                self.env.process(self._integration(last_letter=first_letter - 2,new_fixation_point=prev_pos, new_fixation_point2=new_fixation_point, elem=elem, elem_for_attention=prev_elem, next_elem=next_elem))
