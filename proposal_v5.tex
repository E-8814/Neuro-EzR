...
parameters at the word level (i.e., one predicted value of FFD, gaze
duration, TRT, and skip rate for each individual word position, rather than
aggregate statistics over word categories), jointly with a neural language
model encoder
trained end-to-end on human eye-tracking data.

Beyond its methodological contribution, this work tests whether E-Z
Reader's stage decomposition (familiarity check versus lexical access)
survives end-to-end optimization, providing evidence on whether the
dual-stage architecture reflects real psychological structure or hand-tuning
artifacts of grid-search calibration. Eye movements during reading are a
non-invasive behavioral window onto cognitive processing, so constraining
the architecture that produces them informs theories of how readers
coordinate lexical recognition with visual-motor control.

On the applied side, word-level reading-time prediction underpins
reading-difficulty estimation, automatic text simplification, and
accessibility assessment for second-language learners. Cognitive-stage
decomposition makes such models more defensible in clinical and educational
deployments than black-box regression, because it can localize whether
difficulty arises in familiarity check or lexical access.
...
Task on Eye-Tracking Prediction \citep{hollenstein2021cmcl}, which fine-tune
RoBERTa with custom regression heads) while preserving the mechanistic
interpretability of the underlying cognitive architecture.
...
The model is implemented as a discrete event simulation, and its cognitive
parameters are calibrated by grid search against group-averaged fixation
distributions for a specific corpus.
...
data, enabling the cognitive parameters to be fit by gradient descent at the
word level rather than by grid search against group-averaged fixation
distributions.
...
the contribution of end-to-end parameter learning. All cognitive parameters
are fit as a single global set. The third is the full
...
produce FFD, gaze duration, TRT, and skip rate predictions (one predicted
value per word, compared against the participant-averaged behavioral value
for that word). L1 is predicted by the language model, and L2 is computed
as $\delta \cdot$~L1 (the same L2 $= \beta \cdot$~L1 relationship as in
the original). Both quantities then enter the cascade in the same roles
as in \citet{reichle1998toward}: L1 triggers saccade programming toward
the next word, and L2 governs the duration of refixations. The fourth
model is
...
Model performance will be evaluated using mean absolute error (MAE), root
mean squared error (RMSE), and word-level Pearson correlation between
predicted and observed values for each dependent variable, computed
