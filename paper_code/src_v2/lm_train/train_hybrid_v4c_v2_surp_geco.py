"""
Train v4c_v2_surp on GECO.

Same training recipe as v4c_v2 with one extra responsibility: load
TinyLlama precomputed per-word surprisals from a cache file
(produced by precompute_surprisal.py) and pass them into the model's
forward pass.

Usage:
    python train_hybrid_v4c_v2_surp_geco.py --epochs 5 --seed 42
"""
...
from model_llama_hybrid_v4c_v2_surp import NeuralEZReaderHybrid
...
def _surprisal_tensor(sd_or_agg, surp_cache):
    """
    Look up precomputed surprisal for one sentence.

    sd_or_agg: a SentenceData (per-participant) or AggregatedSentence
        — both have .text_id and .sentence_number (or similar).
    surp_cache: dict from (text_id, sentence_number) -> np.ndarray.
    """
    key = (sd_or_agg.text_id, getattr(sd_or_agg, "sentence_number", 0))
    if key in surp_cache:
        sp = surp_cache[key]
    else:
        # Fallback: zeros (means alpha3 contribution is 0 for this sentence).
...
def compute_loss(pred, h_trt, h_ffd, h_gaze, h_skip, delta, skip_min, skip_max):
...
def save_best(model, save_dir, epoch, val_step, val, model_name, freeze_layers):
    torch.save({
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v4c_v2_surp/geco_tinyllama",
    log_path="../../logs/train_hybrid_v4c_v2_surp_geco.log",
    seed=42,
    ...
    # Load surprisal caches
    print("Loading TinyLlama surprisal caches...")
    surp_train = torch.load(
        os.path.join(cache_dir, "tinyllama_surprisal_geco_train.pt"),
        weights_only=False,
    )
    # The training-loop uses RAW per-participant data (not aggregated), so
    # surprisal cache is keyed by (text_id, sentence_number). We will use
    # the same cache for raw and aggregated as long as keys match.
    surp_val = torch.load(
        os.path.join(cache_dir, "tinyllama_surprisal_geco_val.pt"),
        weights_only=False,
    )
    surp_test = torch.load(
        os.path.join(cache_dir, "tinyllama_surprisal_geco_test.pt"),
        weights_only=False,
    )
    # Combine for training-loop lookups: train_raw uses train_ids, val_raw uses val_ids.
    surp_combined = {}
    surp_combined.update(surp_train)
    surp_combined.update(surp_val)
    surp_combined.update(surp_test)
    print(f"  {len(surp_combined)} sentences cached")

    # Compute skip prior bounds (data-anchored)
    ...
    print(f"  Skip prior bounds: [{skip_min:.3f}, {skip_max:.3f}]")
    ...
    print("\n" + "=" * 100)
    print(f"v4c_v2_surp: ctx_head replaced by + alpha3 * tinyllama_surprisal")
    print(f"  alpha3 init = {model.alpha3.item():.2f}")
    print("=" * 100)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step in range(n_batches):
            ...
            if (step + 1) % val_every == 0 or is_last:
                total_val_steps += 1
                val = evaluate_detailed(model, val_agg, device, subtlex, surp_combined)
    ...
    if test_agg:
        ...
        test = evaluate_detailed(model, test_agg, device, subtlex, surp_combined)
        print(f"\nTest set results:")
...
if __name__ == "__main__":
    ...
    log_path = os.path.join(
        ...
        f"train_hybrid_v4c_v2_surp_geco_seed{args.seed}.log",
    )
    train(
