"""
Neural EZ Reader Hybrid v4c_v2_dualctx — v4c_v2 with TWO specialized ctx heads.

Motivation: in v4c_v2, a single `ctx_head` outputs a per-word
correction added to base_L1. base_L1 is then used in two places:
  (a) Current-word L1 (drives FFD/Gaze/TRT)
  (b) Next-word parafoveal preview (drives skip race)

This couples FFD optimization to skip optimization through the shared
ctx_head: improving (a) for hard-to-process words also affects (b)'s
view of next-word parafoveal preview, which controls skip predictions.
Empirically this manifested as an FFD↔skip trade-off: in the
skipfreeze experiment, r_FFD rose from 0.15 → 0.19 while r_skip
dropped from 0.47 → 0.39 — equal-magnitude trade.

Dual-ctx breaks the coupling:
  - `ctx_head_FFD`(LLaMA): adjusts base_L1 used for the current word's
    L1 calculation (drives FFD, Gaze, TRT, L2).
  - `ctx_head_skip`(LLaMA): adjusts a SEPARATE base_L1 used for the
    parafoveal preview L1 in the skip race.

Both heads see the same projected LLaMA hidden state. They specialize
during training: one for "how long does this word take when fixated",
the other for "how easy is this word to skip when previewed."

Reichle-cog scalars unchanged: α1, α2, ε, δ, M1, M2, I (tied to M2),
λ_refix, refix_pivot, skip_temperature. The two ctx_heads share the
formula skeleton (α1 + α2·log_freq); only the LLaMA-conditioned
correction differs.

Cascade: same as v4c_v2 except the parafoveal-preview L1 path uses
`base_L1_skip` instead of `base_L1` (= `base_L1_FFD`).
...
# --------------------------------------------------------------------------- #
#  Reichle EZ Reader cascade — v4c_v2 + dual base_L1 paths
# --------------------------------------------------------------------------- #


class ReichleEZReaderDualCtx(nn.Module):
    """
    Same cascade math as v4c_v2's ReichleEZReader, except the cascade
    takes TWO base_L1 inputs:
      - base_L1_FFD: drives current-word L1 (FFD path)
      - base_L1_skip: drives next-word parafoveal preview (skip race)

    L2 is computed from base_L1_FFD (it's also a current-word property).
    """
...
# --------------------------------------------------------------------------- #
#  Neural EZ Reader Hybrid v4c_v2_dualctx
# --------------------------------------------------------------------------- #


class NeuralEZReaderHybrid(nn.Module):
    """
    Same neural backbone as v4c_v2. Two ctx heads instead of one.
    ...
    def __init__(
        ...
        # Same architecture as v4c_v2's ctx_head; just two of them.
