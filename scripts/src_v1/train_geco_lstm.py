"""
GPU-efficient training for the Differentiable Neural EZ Reader (LSTM) on GECO.

Key differences from train_geco_lstm.py:
  - Pre-tensorizes all data at startup (no per-sample Python→tensor overhead)
  - Batches sentences together with padding + masks (batch_size=64)
  - Masked loss so padded positions don't affect gradients
  - Batched evaluation too
  - ~10-20x faster per epoch on GPU
"""
...
class AggDataset(Dataset):
    ...
    def __init__(self, agg_list, vocab):
        self.samples = []
        self.agg_refs = agg_list  # keep for printing
        for agg in agg_list:
...
def compute_loss_masked(pred, h_trt, h_ffd, h_gaze, h_skip, mask):
    ...
    """
    # Flatten to 1D over valid positions
    m = mask.flatten()
...
# --------------------------------------------------------------------------- #
#  Print helpers (unbatched, for a few sample sentences)
# --------------------------------------------------------------------------- #
...
def train(
    ...
    batch_size=64,
    lr=1e-3,
    save_dir="../checkpoints_v1/geco_lstm",
    seed=42,
    num_workers=2,
    gpu=0,
):
    ...
    print("\n" + "=" * 90)
    print("Training (LSTM + Differentiable EZ Reader) on GECO Corpus [GPU-batched]")
    print("=" * 90)
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.path.dirname(__file__), "..", "checkpoints_v1/geco_lstm")
