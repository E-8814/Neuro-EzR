...
Changes from v1:
  - LSTM now predicts skip probability (not computed from predictability alone)
  - L2 has a distinct role (integration penalty depends on L2 specifically)
...
class NeuralEZReader(nn.Module):
    """
    End-to-end model:
        tokens + predictability → LSTM → (L1, L2, skip_prob) → DiffEZReader → reading times

    The LSTM predicts three things per word:
      - L1: familiarity check time (early word recognition)
      - L2: lexical access time (full word identification)
      - skip_prob: probability of skipping the word
    """
    ...
    def __init__(self, vocab_size, embedding_dim=64, hidden_dim=128):
        ...
        # This replaces the old sigmoid(predictability) approach
    ...
    def forward(self, token_ids, predictability, word_lengths):
        ...
        Args:
            ...
            predictability: (batch, seq_len) float tensor (0-1) - still passed for info
