"""
Training for model_llama_faithful_sh on GECO corpus — Huber loss variant.

Same as train_faithful_sh_geco.py but replaces MSE with Huber loss for
reading time metrics (TRT, FFD, Gaze).

Huber loss:
  - Errors < delta (50ms): quadratic (like MSE) — precise for small errors
  - Errors > delta (50ms): linear (like MAE) — robust to outlier fixations
  This prevents 500-1000ms outlier fixations from dominating the gradient
  and causing variance collapse.

Usage:
  python3 -u src_v2/lm_train/train_faithful_sh_huber_geco.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
"""
...
L1_MIN = 60.0
SKIP_TARGET = 0.45
HUBER_DELTA = 50.0     # transition point: MSE below, MAE above (ms)
...
# --------------------------------------------------------------------------- #
#  Loss (Huber for reading times, BCE for skip)
# --------------------------------------------------------------------------- #
...
def compute_loss(pred, human_trt, human_ffd, human_gaze, human_skip, delta):
    ...
    if fixated.sum() > 0:
        trt_loss = F.huber_loss(pred_trt[fixated], human_trt[fixated], delta=HUBER_DELTA)
        ffd_loss = F.huber_loss(pred_ffd[fixated], human_ffd[fixated], delta=HUBER_DELTA)
        gaze_loss = F.huber_loss(pred_gaze[fixated], human_gaze[fixated], delta=HUBER_DELTA)
    else:
    ...
    # Skip loss: BCE on all words (unchanged)
...
def print_model_params(model):
    ...
    print(f"  Loss: Huber (delta={HUBER_DELTA}ms)")
...
def train(
    ...
    save_dir="../../checkpoints/faithful_sh_huber/geco_llama",
    log_path="../../logs/train_faithful_sh_huber_geco.log",
    seed=42,
    ...
    print(f"  Learnable cognitive:  delta, l1_scale, skip_head")
    ...
    print("\n" + "=" * 90)
    print(f"Training FAITHFUL+SH model on GECO (HUBER LOSS)")
    print(f"  Model: {model_name}")
    print(f"  Loss: Huber(delta={HUBER_DELTA}ms) for TRT/FFD/Gaze + BCE for skip")
    print(f"  Weights: 0.25*TRT + 0.25*FFD + 0.25*Gaze + 0.4*skip_BCE")
    ...
    print(f"  Skip prior: mean_skip → {SKIP_TARGET} (λ={LAMBDA_PRIOR})")
    print("=" * 90)
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.path.dirname(__file__), "..", "..", "checkpoints", "faithful_sh_huber", f"geco_{model_short}")
    log_path = os.path.join(os.path.dirname(__file__), "..", "..", "logs", f"train_faithful_sh_huber_geco.log")
