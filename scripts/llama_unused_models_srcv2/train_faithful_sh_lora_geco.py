"""
Training for model_llama_faithful_sh_lora on GECO corpus.

Same as train_faithful_sh_geco.py but with two key changes:
  1. LoRA adapters on all LLaMA attention layers (instead of unfreezing top N)
  2. Cosine schedule with warmup (instead of ReduceLROnPlateau)

These address the epoch-1 convergence plateau:
  - LoRA lets all 22 layers adapt with ~2M params at higher LR
  - Cosine schedule prevents premature LR death

Usage:
  python3 -u src_v2/lm_train/train_faithful_sh_lora_geco.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
"""
...
from model_llama_faithful_sh_lora import NeuralEZReaderLLaMA
...
def get_cosine_with_warmup(optimizer, warmup_steps, total_steps, min_lr_ratio=0.01):
...
def print_model_params(model):
    ...
    print(f"    LoRA rank={model.lora_rank}, alpha={model.lora_alpha}, modules={model.n_lora_modules}")
...
def train(
    ...
    warmup_epochs=3,
    save_dir="../../checkpoints/faithful_sh_lora/geco_llama",
    log_path="../../logs/train_faithful_sh_lora_geco.log",
    seed=42,
    ...
    lora_param_list = list(model.lora_params())
    head_param_list = list(model.head_params())

    n_lora_trainable = sum(p.numel() for p in lora_param_list)
    n_head_trainable = sum(p.numel() for p in head_param_list)
    n_frozen = sum(p.numel() for p in model.parameters() if not p.requires_grad)
    total_params = sum(p.numel() for p in model.parameters())

    print(f"  Total parameters:    {total_params:,}")
    print(f"  Frozen (LLaMA base): {n_frozen:,}")
    print(f"  Trainable (LoRA):    {n_lora_trainable:,} (lr={lora_lr})")
    print(f"  Trainable (heads):   {n_head_trainable:,} (lr={head_lr})")
    print(f"  LoRA modules:        {model.n_lora_modules}")
    print(f"  Initial delta:       {model.delta.item():.4f}")

    optimizer = optim.AdamW([
        {"params": lora_param_list, "lr": lora_lr, "weight_decay": 0.01},
        {"params": head_param_list, "lr": head_lr, "weight_decay": 0.0},
    ])

    # Cosine schedule with warmup
    n_batches_per_epoch = (len(train_raw) + batch_size - 1) // batch_size
    steps_per_epoch = (n_batches_per_epoch + accumulation_steps - 1) // accumulation_steps
    ...
    print("\n" + "=" * 90)
    print(f"Training FAITHFUL+SH+LoRA model on GECO")
    print(f"  Model: {model_name}")
    print(f"  LoRA: rank={lora_rank}, alpha={lora_alpha}, on q/k/v/o_proj")
    print(f"  FFD = L1 | Gaze = L1 + L2 | TRT = Gaze + regression (conditional)")
    print(f"  Loss: 0.25*TRT + 0.25*FFD + 0.25*Gaze + 0.4*skip_BCE")
    print(f"  Batch: {batch_size} | Accum: {accumulation_steps} | Effective: {batch_size * accumulation_steps}")
    print(f"  Schedule: cosine with {warmup_epochs}-epoch warmup ({warmup_steps} steps)")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        lora_lr_now = optimizer.param_groups[0]['lr']
        head_lr_now = optimizer.param_groups[1]['lr']

        is_best = val['r_trt'] > best_val_corr

        print(f"\n[Epoch {epoch:3d}/{num_epochs}] {elapsed:.1f}s | step={global_step} | "
              f"lora_lr={lora_lr_now:.2e} head_lr={head_lr_now:.2e}")
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.path.dirname(__file__), "..", "..", "checkpoints", "faithful_sh_lora", f"geco_{model_short}")
    log_path = os.path.join(os.path.dirname(__file__), "..", "..", "logs", f"train_faithful_sh_lora_geco.log")
