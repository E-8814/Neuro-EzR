"""
Neural EZ Reader Model (LLaMA + Differentiable EZ Reader) - v3.

Two key changes from v2:

1. L2 = delta * L1, matching the original E-Z Reader theory (Reichle et al.,
   1998-2012). Instead of two independent heads, a single head predicts L1
   and L2 is derived as a learnable fraction of L1.
       t(L2) = delta * t(L1)
   with delta ranging from 0.25 to 0.85 across published model versions.

2. Uses DifferentiableEZReaderV3: reading times (TRT, FFD, Gaze) are
   conditional on fixation. Skip is a separate output, not multiplied into
   TRT. This fixes the systematic under-prediction in v2.

Uses a causal (left-to-right) language model for cognitive plausibility:
when processing word N, the model only sees words 1..N.
"""
...
from diff_ezreader_v3 import DifferentiableEZReaderV3


class NeuralEZReaderLLaMAv3(nn.Module):
    """
    End-to-end model:
        word tokens -> LLaMA (causal) -> word-level pooling -> L1 head
        -> L2 = delta * L1 (theory-constrained)
        -> skip head
        -> DifferentiableEZReaderV3 -> (TRT|fixated, FFD|fixated, Gaze|fixated, skip)
    """
    ...
    def __init__(
        ...
        # Single head predicts L1; L2 is derived via delta (see forward)
        ...
        # Initialize at 0.34 (latest published value)
        # Stored in unconstrained space; sigmoid maps it to (0, 1) in forward
        self._delta_raw = nn.Parameter(torch.tensor(0.0))  # sigmoid(0) = 0.5, will be learned
        # Initialize so that sigmoid(_delta_raw) ≈ 0.34
        with torch.no_grad():
        ...
        # Reading times are conditional on fixation (no skip multiplication)
        ezr_ablation = ablation if ablation in ('no_eccentricity', 'no_regressions', 'ffd_l1_only') else None
        self.ezreader = DifferentiableEZReaderV3(ablation=ezr_ablation)
    ...
    def forward(self, word_lists, predictability, word_lengths):
        ...
        # --- Derive L2 from L1 via delta (matching original E-Z Reader theory) ---
        # t(L2) = delta * t(L1), where delta in (0, 1) is a learned scalar
        if self.ablation == 'no_two_stage':
            # Ablation: no decomposition, L2 = 0 (everything is L1)
        ...
        # --- Differentiable EZ Reader v3 produces reading metrics ---
