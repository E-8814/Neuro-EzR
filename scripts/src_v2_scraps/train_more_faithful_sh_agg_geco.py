"""
Training for model_llama_more_faithful_sh on GECO corpus — AGGREGATED training.

Same as train_more_faithful_sh_geco.py but trains on aggregated (averaged
across participants) data instead of individual per-participant data.

Differences:
  - Training loop uses train_agg (one sample per sentence) instead of
    train_raw (14 samples per sentence, one per participant)
  - Targets are averaged reading times and skip rates (soft labels)
  - Fewer training examples per epoch (~3700 vs ~55000) but cleaner targets

Usage:
  python3 -u src_v2/lm_train/train_more_faithful_sh_agg_geco.py
  python3 -u src_v2/lm_train/train_more_faithful_sh_agg_geco.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
"""
...
from model_llama_more_faithful_sh import NeuralEZReaderLLaMA
...
def compute_loss(pred, human_trt, human_ffd, human_skip, delta):
    pred_trt = pred['conditional_trt'].float()
    pred_ffd = pred['first_fixation'].float()
    pred_skip = pred['skip_prob'].float()

    # Mask: words where most participants fixated
...
def evaluate_detailed(model, agg_data, device, batch_size=8):
    ...
    with torch.no_grad():
        for i in range(0, len(agg_data), batch_size):
            ...
            word_lists, wlens, h_trt, h_ffd, h_gaze, h_skip = collate_aggregated(batch, device)
...
def print_model_params(model):
    ...
    print(f"    skip_head                 = learned (from LLaMA repr)")
    print(f"  From theory (fixed):")
    print(f"    FFD                       = L1 (Reichle et al. 2003)")
    print(f"    Gaze                      = L1 + L2 (Reichle et al. 2003)")
    print(f"  Integration failure (learnable):")
    print(f"    regression_sharpness      = {ezr.regression_sharpness.item():.4f} (init=0.03)")
    print(f"    regression_threshold      = {ezr.regression_threshold.item():.1f}ms (init=100.0)")
    print(f"    regression_cost_scale     = {ezr.regression_cost_scale.item():.4f} (init=0.25)")
...
def train(
    ...
    save_dir="../../checkpoints/more_faithful_sh_agg/geco_llama",
    log_path="../../logs/train_more_faithful_sh_agg_geco.log",
    seed=42,
    ...
    print(f"  Train words: {n_train_words} | Val words: {n_val_words} | Test words: {n_test_words}")
    ...
    print(f"  Learnable cognitive:  delta, l1_scale, skip_head, regression params")
    ...
    print("\n" + "=" * 90)
    print(f"Training MORE FAITHFUL+SH model on GECO (AGGREGATED)")
    print(f"  Model: {model_name}")
    print(f"  Training on AGGREGATED data ({len(train_agg)} sentences, ~{n_train_words} words)")
    print(f"  Contribution: L1 = neural_net(context), L2 = delta*L1, skip = learned head")
    print(f"  FFD = L1 | Gaze = L1 + L2 | TRT = Gaze + regression (conditional)")
    print(f"  Loss: 0.33*TRT(cond) + 0.33*FFD + 0.4*skip_BCE + delta_reg")
    print(f"  Time losses masked to fixated words (skip_rate < 0.5) | skip loss on all words")
    print(f"  Regression parameters: learnable (init from literature)")
    print(f"  Batch: {batch_size} | Accum: {accumulation_steps} | Effective: {batch_size * accumulation_steps}")
    print(f"  AMP: {use_amp}")
    print(f"  Regularizers: delta_range(0.20-0.50, λ={LAMBDA_DELTA})")
    print("=" * 90)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step in range(n_batches):
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                pred = model(word_lists, wlens)
            loss, parts = compute_loss(pred, h_trt, h_ffd, h_skip, model.delta)
        ...
        print(f"  Train: loss={epoch_loss:.1f} "
              ...
              f"skip={epoch_skip:.3f}) | {n_samples:,} samples")
        ...
        ezr = model.ezreader
        print(f"  Regression: sharpness={ezr.regression_sharpness.item():.4f}  "
              f"threshold={ezr.regression_threshold.item():.1f}ms  "
              f"cost_scale={ezr.regression_cost_scale.item():.4f}")
        ...
        if is_best:
            ...
            torch.save({
                ...
                'regression_sharpness': ezr.regression_sharpness.item(),
                'regression_threshold': ezr.regression_threshold.item(),
                'regression_cost_scale': ezr.regression_cost_scale.item(),
                'val_metrics': val,
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.path.dirname(__file__), "..", "..", "checkpoints", "more_faithful_sh_agg", f"geco_{model_short}")
    log_path = os.path.join(os.path.dirname(__file__), "..", "..", "logs", f"train_more_faithful_sh_agg_geco.log")
