"""
Training for model_llama_v3_cond on Provo corpus (fast iteration).

Key changes from train_delta_rem_p_comp.py:
  1. TRT loss uses conditional_trt (gaze + regression) instead of
     total_reading_time ((1-skip) * gaze). Human TRT is conditional on
     fixation, so the (1-skip) factor was creating systematic underprediction.
  2. l1_scale is learnable (was fixed at 50).
  3. refix_threshold is learnable (was fixed at 150ms, init 200ms).
  4. Delta regularization: soft penalty outside [0.20, 0.50].
  5. Removed trt_scale regularizer (no longer needed without (1-skip) mismatch).
  6. L1 bounds widened: 60-400ms (L1 should match human FFD scale now).

Usage:
  python3 -u src_v2/lm_train_provo/train_v3_cond_provo.py
  python3 -u src_v2/lm_train_provo/train_v3_cond_provo.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
"""
...
from model_llama_v3_cond import NeuralEZReaderLLaMA
...
# --------------------------------------------------------------------------- #
#  Loss — the key change: TRT loss on conditional_trt, not total_reading_time
# --------------------------------------------------------------------------- #

def compute_loss(pred, human_trt, human_ffd, human_gaze, human_skip, delta):
    # Conditional TRT = gaze + regression (no skip factor).
    # Human TRT is conditional on fixation, so this is the right target.
    ...
    trt_loss = F.mse_loss(pred_trt, human_trt)
    ffd_loss = F.mse_loss(pred_ffd, human_ffd)
    gaze_loss = F.mse_loss(pred_gaze, human_gaze)
    ...
    # Delta regularization: soft penalty outside [0.20, 0.50]
...
# --------------------------------------------------------------------------- #
#  Evaluation — uses conditional_trt for TRT metrics
# --------------------------------------------------------------------------- #
    ...
    with torch.no_grad():
        for i in range(0, len(agg_data), batch_size):
            ...
            for b in range(len(batch)):
                ...
                # Use conditional_trt for TRT eval (matches human conditional data)
...
def print_model_params(model):
    ...
    print(f"    delta (L2/L1 ratio)       = {delta:.4f} (init=0.34, target=0.20-0.50)")
    print(f"    l1_scale                  = {model.l1_scale.item():.2f} (learnable, init=50)")
    print(f"    refix_threshold           = {ezr.refix_threshold.item():.1f}ms (learnable, init=200)")
...
def train(
    ...
    save_dir="../../checkpoints/v3_cond/provo_llama",
    log_path="../../logs/train_v3_cond_provo.log",
    seed=42,
    ...
    print(f"  Words: {n_train_words} train | {n_val_words} val | {n_test_words} test")
    ...
    # ---- Optimizer: 2 groups (LM vs heads+cognitive params) ----
    lm_params = []
    head_params = []
    ...
    for name, param in model.named_parameters():
        ...
        if name.startswith("llama."):
            ...
            # projection, l1_head, skip_head, _delta_raw, l1_scale, ezreader.refix_threshold
    ...
    print(f"  Learnable cognitive:  delta, l1_scale, refix_threshold")
    ...
    print("\n" + "=" * 90)
    print(f"Training v3_cond (conditional TRT, learnable l1_scale+refix_threshold) on Provo")
    print(f"  Model: {model_name}")
    print(f"  Architecture: L1 head + skip head + delta + l1_scale + refix_threshold")
    print(f"  FFD = L1 | Gaze = L1 + refix_prob * L2")
    print(f"  TRT loss on conditional_trt = Gaze + regression (NOT (1-skip)*gaze)")
    ...
    print(f"  Regularizers: skip_prior(lambda={LAMBDA_PRIOR}, target={SKIP_TARGET}) + "
          f"l1_range({L1_MIN}-{L1_MAX}ms) + delta_range(0.20-0.50, lambda={LAMBDA_DELTA})")
    print(f"  Loss: 0.25*TRT(cond) + 0.25*FFD + 0.25*Gaze + 0.4*Skip + regularizers")
    print("=" * 90)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f"  Cog:   l1_scale={model.l1_scale.item():.2f}  "
              f"refix_thresh={model.ezreader.refix_threshold.item():.1f}ms")
        ...
        if is_best:
            ...
            torch.save({
                ...
                'refix_threshold': model.ezreader.refix_threshold.item(),
                'val_metrics': val,
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.path.dirname(__file__), "..", "..", "checkpoints", "v3_cond", f"provo_{model_short}")
    log_path = os.path.join(os.path.dirname(__file__), "..", "..", "logs", f"train_v3_cond_provo.log")
