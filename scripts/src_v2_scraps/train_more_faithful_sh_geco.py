"""
Training for model_llama_more_faithful_sh on GECO corpus.

Changes from train_faithful_sh_geco.py:
  - Predictability removed from collate (model no longer takes it)
  - Simplified loss: TRT + FFD + Skip + delta regularizer
    (dropped Gaze loss, skip prior, L1 soft bounds)
  - Regression parameters are learnable (logged each epoch)

Usage:
  python3 -u src_v2/lm_train/train_more_faithful_sh_geco.py
  python3 -u src_v2/lm_train/train_more_faithful_sh_geco.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
"""
...
def collate_sentences(batch, device):
    ...
    return word_lists, wlens, h_trt, h_ffd, h_skip
...
def compute_loss(pred, human_trt, human_ffd, human_skip, delta):
    ...
    # Mask: only fixated words for time losses
...
def train(
    ...
    save_dir="../../checkpoints/more_faithful_sh/geco_llama",
    log_path="../../logs/train_more_faithful_sh_geco.log",
    seed=42,
    ...
    print("\n" + "=" * 90)
    print(f"Training MORE FAITHFUL+SH model on GECO")
    ...
    print(f"  Time losses masked to fixated words | skip loss on all words")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step in range(n_batches):
            ...
            word_lists, wlens, h_trt, h_ffd, h_skip = collate_sentences(batch, device)
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.path.dirname(__file__), "..", "..", "checkpoints", "more_faithful_sh", f"geco_{model_short}")
    log_path = os.path.join(os.path.dirname(__file__), "..", "..", "logs", f"train_more_faithful_sh_geco.log")
