"""
Training for LLaMA + Differentiable EZ Reader v3 on GECO corpus.

v3 key changes:
  1. L2 = delta * L1 (matching original E-Z Reader theory)
  2. Reading times are conditional on fixation (no skip multiplication in TRT)
  3. TRT/FFD/Gaze losses are masked for skipped words (loss only on fixated words)
  4. No L2 regularizers needed (L2 = delta * L1, structurally bounded)

Usage:
  python3 -u src_v2/train_geco_llama_v3.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
  python3 -u src_v2/train_geco_llama_v3.py --ablation no_two_stage
"""
...
# No L2 regularizer needed: L2 = delta * L1, so L2 is bounded if L1 is bounded.
...
# --------------------------------------------------------------------------- #
#  Loss function (v3: no L2 regularizer, balanced weights)
# --------------------------------------------------------------------------- #
...
def compute_loss(pred, human_trt, human_ffd, human_gaze, human_skip):
    ...
    v3 changes from v2:
      - TRT/FFD/Gaze losses are MASKED for skipped words. The model predicts
        reading time conditional on fixation, so loss should only be computed
        on words that were actually fixated (where target > 0).
      - Skip loss applies to all words (skip is a separate prediction).
      - No L2 floor regularizer (L2 = delta * L1, structurally bounded).
    """
    ...
    # Mask: only compute reading time losses on fixated words.
    # For per-participant data: skipped words have TRT=0, FFD=0, Gaze=0.
    # For aggregated data: mean_trt/ffd/gaze exclude skips (always > 0 if fixated).
    fixated = (human_trt > 0).float()
    n_fixated = fixated.sum().clamp(min=1.0)

    # Masked MSE: average only over fixated words
    trt_loss = (fixated * (pred_trt - human_trt) ** 2).sum() / n_fixated
    ffd_loss = (fixated * (pred_ffd - human_ffd) ** 2).sum() / n_fixated
    gaze_loss = (fixated * (pred_gaze - human_gaze) ** 2).sum() / n_fixated

    # Skip loss applies to ALL words (both skipped and fixated)
    ...
    # TRT scale matching: only over fixated words
    pred_trt_fixated_mean = (fixated * pred_trt).sum() / n_fixated
    human_trt_fixated_mean = (fixated * human_trt).sum() / n_fixated
    trt_scale = LAMBDA_TRT_SCALE * (pred_trt_fixated_mean - human_trt_fixated_mean) ** 2
...
def print_ezreader_params(model):
    ...
    print("  Learned EZ Reader v3 parameters:")
...
def train(
    ...
    save_dir="../../checkpoints/v3/geco_llama",
    seed=42,
    ...
    for name, param in model.named_parameters():
        ...
        if name.startswith("llama."):
            ...
            # delta is a cognitive architecture parameter, group with EZR
    ...
    print(f"Training v3 (LLaMA + DiffEZR, L2=delta*L1) on GECO{ablation_label}")
    print(f"  Model: {model_name}")
    print(f"  v3 change: L2 = delta * L1 (single head, theory-constrained)")
...
if __name__ == "__main__":
    ...
    if args.ablation:
        save_dir = os.path.join(os.path.dirname(__file__), "..", "..",
                                "checkpoints", "v3", f"geco_{model_short}_ablation_{args.ablation}")
    else:
        save_dir = os.path.join(os.path.dirname(__file__), "..", "..", "checkpoints", "v3", f"geco_{model_short}")
