...
studies. Moreover, the cognitive parameters governing the cascade --- the
timing constants of L1, L2, saccade programming, motor execution, and
integration failure --- are estimated by grid search against group-averaged
fixation distributions, because the simulation is non-differentiable and
cannot be fit by gradient descent.
...
divergence informs future cognitive theories of eye-movement control.

\noindent\textbf{Societal relevance.} Word-level reading-time prediction
underpins applications in reading-difficulty estimation, dyslexia screening,
automatic text simplification, and accessibility assessment for second-language
learners. Models that combine quantitative accuracy with interpretable
cognitive stages are more defensible in clinical and educational deployments
than black-box regression models, because stage-level decomposition can
localize where in the reading process --- familiarity, lexical access, or
saccade planning --- a particular text or reader population is encountering
difficulty.
...
comparable to (i) the original simulation-based E-Z Reader
\citep{reichle1998toward} with grid-searched parameters, and (ii) a
matched-capacity neural regression baseline that uses the same encoder with
four independent output heads predicting FFD, gaze duration, TRT, and skip
rate directly, while preserving the mechanistic interpretability of the
