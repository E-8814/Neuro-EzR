"""
GECO-trained Model Comparison + Psycholinguistic Effects Analysis (v2).
...
def bin_predictability(words):
    ...
    for w in words:
        if w['pred'] <= lo:     bins['Low pred'].append(w)
        elif w['pred'] <= hi:   bins['Med pred'].append(w)
...
def main():
    ...
    output_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'comparison_geco_v2_results.txt')
    sys.stdout = Logger(output_path)

    data_dir = os.path.join(os.path.dirname(__file__), '..', 'data')
    geco_lstm_ckpt = os.path.join(os.path.dirname(__file__), '..', '..', 'checkpoints', 'v2', 'geco_lstm', 'best_model_lstm.pt')
    geco_bert_ckpt = os.path.join(os.path.dirname(__file__), '..', '..', 'checkpoints', 'v2', 'geco_bert', 'best_model_bert.pt')
    geco_llama_ckpt = os.path.join(os.path.dirname(__file__), '..', '..', 'checkpoints', 'v2', 'geco_TinyLlama_TinyLlama-1.1B-Chat-v1.0', 'best_model.pt')
    geco_direct_ckpt = os.path.join(os.path.dirname(__file__), '..', '..', 'checkpoints', 'v2', 'geco_direct_TinyLlama_TinyLlama-1.1B-Chat-v1.0', 'best_model.pt')
    ...
    print(f"{'=' * W}")
    print(f"  V2 MODEL COMPARISON + EFFECTS ANALYSIS")
    print(f"  Models trained with v2 pipeline, evaluated on GECO test + full Provo")
    print(f"{'=' * W}")
    ...
    # ================================================================
    #  Load models (v2-trained)
    # ================================================================
    print("\nLoading v2-trained models...")
    ...
    lstm_model.eval()
    print(f"  LSTM v2: loaded (epoch {ckpt_lstm.get('epoch', '?')})")
    ...
    try:
        ...
        bert_model.eval()
        print(f"  BERT v2: loaded (epoch {ckpt_bert.get('epoch', '?')})")
    ...
    for name, key in [
        ...
        ('Diff EZ Reader v2', 'diff_trt'),
        ('Neural EZ Reader v2 (LSTM)', 'lstm_trt'),
    ]:
