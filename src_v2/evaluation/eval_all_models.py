...
Metrics: Pearson r, MAE, RMSE, Bias for TRT, FFD, Gaze, Skip.
Also reports corrected TRT for v2 models (TRT / (1-skip)).

Usage:
  python3 -u src_v2/eval_all_models.py
  CUDA_VISIBLE_DEVICES=1 python3 -u src_v2/eval_all_models.py
"""
...
SRC_V2_DIR = os.path.join(SCRIPT_DIR, '..')
...
sys.path.insert(0, os.path.join(SRC_V2_DIR, 'lm_model'))
sys.path.insert(0, os.path.join(SRC_V2_DIR, 'model'))
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default=os.path.join(ROOT_DIR, "results/eval_all_models_results.txt"))
    ...
    ckpt = torch.load(os.path.join(ROOT_DIR, "checkpoints", "v2", "geco_TinyLlama_TinyLlama-1.1B-Chat-v1.0", "best_model.pt"),
    ...
    v2delta_ckpt_path = os.path.join(ROOT_DIR, "checkpoints", "v2_delta", "geco_TinyLlama_TinyLlama-1.1B-Chat-v1.0", "best_model.pt")
    ...
    ckpt = torch.load(os.path.join(ROOT_DIR, "checkpoints", "v2", "geco_direct_TinyLlama_TinyLlama-1.1B-Chat-v1.0", "best_model.pt"),
                       map_location=device, weights_only=False)
    ...
    # Note: The checkpoint is NeuralEZReaderBERT, not BertDirectRegression.
    # BertDirectRegression has not been trained. This is BERT with the cognitive architecture.
    print("[5/8] Loading BERT + EZR...")
    from model_bert import NeuralEZReaderBERT
    ckpt = torch.load(os.path.join(ROOT_DIR, "checkpoints", "v2", "geco_bert", "best_model_bert.pt"),
                       map_location=device, weights_only=False)
    ...
    # Older checkpoint: has ffd_offset (removed in v2), missing skip_head (added in v2)
