...
Changes from v1:
  - FFD formula: L1_scaled + softplus(l2_contribution) * L2_scaled + ffd_offset
  - Explicit regression mechanism: sigmoid threshold on L2
  - Removed integration_cost parameter
  - New parameters: ffd_offset, l2_contribution, regression_threshold,
...
class DifferentiableEZReader(nn.Module):
    ...
    def __init__(self):
        ...
        # v2 new parameters
        self.ffd_offset = nn.Parameter(torch.tensor(100.0))               # motor latency baseline (ms)
    ...
    def forward(self, L1, L2, skip_input, word_lengths, input_is_prob=False):
        ...
        # --- 4. First fixation duration (v2: includes L2 contribution + offset) ---
        first_fixation = L1_scaled + torch.nn.functional.softplus(self.l2_contribution) * L2_scaled + self.ffd_offset

        # --- 5. Gaze duration (first-pass reading) ---
        gaze_duration = L1_scaled + L2_scaled

        # --- 6. Regression mechanism (v2: explicit sigmoid threshold on L2) ---
        regression_prob = torch.sigmoid(
            self.regression_sharpness * (L2 - self.regression_threshold)
        )
        # Regression cost: probability * cost_scale * previous word's gaze duration
        # Shift gaze_duration right by 1 (first word has no previous word -> 0)
