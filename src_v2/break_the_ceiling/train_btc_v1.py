"""
Training for model_llama_v2_delta_btc_v1 on GECO corpus.

Break The Ceiling v1: more unfrozen LLaMA layers + bigger heads.
Trains on per-participant data (same as v2_delta baseline).

Changes from train_geco_llama_v2_delta.py:
  - Imports model_llama_v2_delta_btc_v1 (bigger heads, 50% freeze)
  - freeze_layers defaults to 50% (auto-detected from model)

Usage:
    python3 -u src_v2/break_the_ceiling/train_btc_v1.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
    python3 -u src_v2/break_the_ceiling/train_btc_v1.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0 --freeze 8
"""
...
from model_llama_v2_delta_btc_v1 import NeuralEZReaderLLaMA
...
def train(
    ...
    save_dir="../../checkpoints/break_ceiling/btc_v1",
    seed=42,
    ...
    actual_freeze = model.freeze_layers
    n_layers = len(model.llama.layers)
    print(f"  Freezing {actual_freeze}/{n_layers} layers ({100*actual_freeze/n_layers:.0f}%)")
    print(f"  Unfrozen LLaMA layers: {n_layers - actual_freeze}")
    ...
    print(f"  Head hidden_dim:       512 (was 256 in v2_delta)")
    ...
    print(f"BTC v1: bigger heads + more unfrozen layers{ablation_label}")
    print(f"  Model: {model_name}")
    print(f"  Frozen layers: {actual_freeze}/{n_layers} ({100*actual_freeze/n_layers:.0f}%) — was 75% in v2_delta")
    print(f"  Head dim: 512 (was 256) | Head depth: 3 layers (was 2)")
    print(f"  Trainable LM params: {n_lm_trainable:,} (was ~{n_lm_trainable * (n_layers - actual_freeze) // max(n_layers - 16, 1) if n_layers > 16 else 0:,} in v2_delta)")
    ...
    print(f"  Baseline (v2_delta, 75% frozen, dim=256): r_TRT=0.467 at epoch 2")
    print("=" * 90)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if is_best:
            ...
            torch.save({
                ...
                'model_name': model_name,
                'freeze_layers': actual_freeze,
                'ablation': ablation,
                'delta': model.delta.item(),
                'val_metrics': val_metrics,
                'model_variant': 'btc_v1',
                'hidden_dim': 512,
    ...
    print(f"Baseline (v2_delta): r_TRT = 0.467")
    print(f"Improvement: {best_val_corr - 0.467:+.3f}")
    print("=" * 90)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--freeze", type=int, default=None,
                        help="Number of layers to freeze (default: 50%% of total)")
    ...
    save_dir = os.path.join(os.path.dirname(__file__), "..", "..",
                            "checkpoints", "break_ceiling", f"btc_v1_{model_short}")
