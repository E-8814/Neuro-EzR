"""
Train model_llama_v2_delta on AGGREGATED GECO data instead of per-participant data.

Hypothesis: the 2-epoch plateau happens because the model trains on 54,930
per-participant observations (same sentence x 14 noisy readings) but evaluates
on participant-averaged data. The model learns the mean in 1-2 passes and then
spends remaining epochs fitting irreducible per-participant noise.

This script trains directly on aggregated (participant-averaged) sentences:
  - ~3,900 training sentences (clean targets)
  - Each sentence appears ONCE per epoch (no noisy duplicates)
  - If r_TRT improves beyond 0.467, per-participant noise was the bottleneck
  - If r_TRT stays at 0.467, we're hitting the data ceiling

Based on: model_llama_v2_delta.py (L2 = delta * L1)
Compared against: train_geco_llama_v2_delta.py (per-participant training)

Usage:
    python3 -u src_v2/break_the_ceiling/train_aggregated.py
    python3 -u src_v2/break_the_ceiling/train_aggregated.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
    python3 -u src_v2/break_the_ceiling/train_aggregated.py --epochs 100 --head_lr 1e-3
"""
...
from model_llama_v2_delta import NeuralEZReaderLLaMA
from data_loader import aggregate_by_sentence
from geco_loader import load_geco, split_geco


# --------------------------------------------------------------------------- #
#  Regularization hyperparameters (same as v2_delta)
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
#  Loss function (same as v2_delta)
# --------------------------------------------------------------------------- #
...
def print_ezreader_params(model):
    """Print the learned EZ Reader parameters + delta."""
    ezr = model.ezreader
    delta = model.delta.item()
    print("  Learned EZ Reader v2-delta parameters:")
...
def train(
    ...
    save_dir="../../checkpoints/break_ceiling/aggregated",
    seed=42,
    ...
    # Split aggregated by text_id (use same splits as per-participant training)
    ...
    print(f"  Aggregated splits: {len(train_agg)} train | {len(val_agg)} val | {len(test_agg)} test")

    n_train_words = sum(len(a.tokens) for a in train_agg)
    n_val_words = sum(len(a.tokens) for a in val_agg)
    print(f"  Train words: {n_train_words:,} | Val words: {n_val_words:,}")
    print(f"  (Compare: per-participant training uses {len(train_raw):,} sentence observations)")
    ...
    print(f"  Trainable (EZR+delta): {n_ezr_trainable:,} (lr={ezr_lr})")
    ...
    print("\n" + "=" * 90)
    print("EXPERIMENT: Train on AGGREGATED data (vs per-participant baseline)")
    print("=" * 90)
    print(f"  Model: {model_name}")
    print(f"  Training data: {len(train_agg)} aggregated sentences ({n_train_words:,} words)")
    print(f"  Baseline training: {len(train_raw):,} per-participant observations")
    print(f"  Hypothesis: plateau at epoch 2 is caused by per-participant noise")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # Shuffle aggregated training data
        ...
        # Print samples every 5 epochs or when best
        if epoch <= 5 or epoch % 5 == 0 or is_best:
    ...
    print(f"Final delta = {model.delta.item():.4f} (literature range: 0.20-0.85)")
    print("=" * 90)
    ...
    # ---- Comparison summary ----
    print("\n" + "=" * 90)
    print("COMPARISON: Aggregated training vs Per-participant training (v2_delta)")
    print("=" * 90)
    print(f"  Per-participant baseline: r_TRT=0.467 (best at epoch 2, plateaus)")
    print(f"  Aggregated training:     r_TRT={best_val_corr:.3f} (best at epoch {epoch})")
    if best_val_corr > 0.475:
        print(f"  RESULT: Aggregated training BREAKS the plateau (+{best_val_corr - 0.467:.3f})")
        print(f"  --> Per-participant noise WAS the bottleneck")
    elif best_val_corr > 0.467:
        print(f"  RESULT: Marginal improvement (+{best_val_corr - 0.467:.3f})")
        print(f"  --> Mixed evidence; noise is part of the problem but not all of it")
    else:
        print(f"  RESULT: No improvement ({best_val_corr - 0.467:+.3f})")
        print(f"  --> Per-participant noise is NOT the bottleneck; hitting data ceiling")
    print("=" * 90)
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.path.dirname(__file__), "..", "..",
                            "checkpoints", "break_ceiling", f"aggregated_{model_short}")
