"""
Per-participant training: train model_llama_faithful_sh on each Provo
participant individually and report results.

Provo has ~84 participants (vs GECO's 14), but only ~134 sentences each.
Each participant has fewer sentences but the data is cleaner (controlled
experimental stimuli vs novel reading).

For each participant:
  1. Filter Provo to that participant's data
  ...
  5. Reset model and repeat

Usage:
  python3 -u src_v2/lm_train_provo/train_faithful_sh_per_participant_provo.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
"""
...
from data_loader import load_provo, split_dataset
...
def train_one_participant(model, train_data, val_data, test_data, participant_id,
                          device, batch_size=8, accumulation_steps=2,
...
def main(model_name="meta-llama/Llama-3.2-1B", freeze_layers=18,
         batch_size=8, accumulation_steps=2, lm_lr=2e-6, head_lr=5e-4,
         seed=42):
    ...
    # Get train/val/test text_id split (same as main experiments)
    ...
    print(f"\nLoading model: {model_name} (freeze={freeze_layers}, lm_lr={lm_lr})")
    ...
    print(f"  lm_lr={lm_lr}, head_lr={head_lr}, freeze={freeze_layers}")

    # ---- Run per-participant experiments ----
    all_results = []

    print("\n" + "=" * 90)
    print(f"PER-PARTICIPANT TRAINING on PROVO ({NUM_EPOCHS} epochs each)")
    ...
    for p_idx, pid in enumerate(participants):
        ...
        if len(p_train) < 20 or len(p_val) < 5:
    ...
    # ---- Summary ----
    print("\n" + "=" * 90)
    print("SUMMARY: Per-participant results on PROVO (test set)")
    print("=" * 90)
    ...
    if all_results:
        ...
        print(f"    Provo faithful_sh: r_TRT ≈ 0.612")
        print(f"\n  Interpretation:")
        print(f"    mean per-participant r_TRT = {mean_r_trt:.3f}")
        if mean_r_trt > 0.55:
            print(f"    >> HIGHER than population model → individual modeling helps")
        elif mean_r_trt > 0.40:
            print(f"    ~~ MODERATE — single-trial noise limits individual prediction")
        else:
            print(f"    << LOWER than population model → too few sentences per participant")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lm_lr", type=float, default=2e-6)
    ...
    if args.freeze is not None:
        ...
        freeze_layers = 18  # freeze 18/22, only top 4 layers trainable
        print(f"Default freeze: {freeze_layers} layers")
