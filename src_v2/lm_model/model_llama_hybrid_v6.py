"""
Neural EZ Reader Hybrid v6 — v5 minus the skip residual head,
plus Reichle-unit alpha1 / alpha2 accessors.

Changes from model_llama_hybrid_v5.py:

  (1) PURE PARAFOVEAL RACE SKIP.
      v5's skip_residual_head is removed. Skip probability is now
        skip_prob = sigmoid((M1 - L1_next_parafoveal) / skip_temperature)
      with no learned correction. This is the cleanest possible
      approximation of Reichle's M1 / L1(n+1) race, and it is the
      scientific claim for the paper: *skip emerges from the cascade
      itself, not from a learned bypass.*

      Expected empirical effect relative to v4 / v5: r_skip likely
      drops by roughly 0.10 (from ~0.67 to ~0.55) because the race
      is noisy. The tradeoff is a much stronger narrative — no MLP
      is needed to make skipping work.

  (2) REICHLE-UNIT PROPERTIES.
      Two new properties expose the L1 formula coefficients in the
      units Reichle actually reported, so paper tables do not need
      inline arithmetic to bridge the normalization.

      We parameterize L1 against a normalized log-frequency feature
      (log f - 10) / 5 with offset and coef scalars, and Reichle's
      2003 formula is
      ...
      Init values (offset = 60, coef = -17) give alpha1_R = 94 ms
      (Reichle: 104) and alpha2_R = 3.4 (Reichle: 3.4).

  (3) CTX HEAD UNCHANGED.
      L1 still has an explicit Reichle skeleton plus
        ctx = ctx_head(LLaMA_hidden)
      as a supra-lexical residual. The paper's Test 1 (check that ctx
      has low correlation with surprisal) remains the fork between
      Claim A ("LLaMA + Reichle fits better") and Claim B
      ("ctx reveals what Reichle's formula is missing"). Dropping
      the skip residual has no effect on this decomposition.

Unchanged from v5:
  - LLaMA backbone is AutoModelForCausalLM; surprisal from lm_head.
  - Explicit predictability term `- alpha3 * exp(-surprisal)` in L1.
  - M2 and I tied through a single _M2I_raw parameter.
  - Soft L1 floor at 5 ms.
  - Eccentricity `eps^((w-1)/2)` for foveal L1; Reichle's full
    `eps^(parafoveal_dist + (w_next-1)/2)` for the skip race.
  - Refixation sigmoid on word length.
  - Regression cost `pF * reg_weight * prev_gaze`.
...
class ReichleEZReader(nn.Module):
    """
    Maps (base_L1, L2, word_lengths) to FFD / Gaze / TRT / skip via the
    Reichle cascade. Skip is produced by the parafoveal M1 / L1_next race
    alone — no learned residual correction.
    """
    ...
    def __init__(self):
        ...
        # M2 and I tied: single parameter, same value in FFD and TRT.
    ...
    def forward(self, base_L1, L2, word_lengths):
        ...
        # --- TRT with integration + regression cost (I tied to M2) ---
        prev_gaze = torch.cat(
        ...
        # --- Pure parafoveal race skip (no residual correction) ---
...
class NeuralEZReaderHybrid(nn.Module):
    ...
    def __init__(
        ...
        # alpha3 analogue: predictability coefficient (ms), applied as
        # `- l1_pred_coef * exp(-surprisal)`. Init 40 ms, close to
        # Reichle 2003's alpha3 = 39 ms/cloze.
        self.l1_pred_coef = nn.Parameter(torch.tensor(40.0))

        # Context-effects encoder: supra-lexical residual on top of the
        # Reichle skeleton. Near-zero init so the formula dominates at
        # epoch 1 and ctx only picks up signal the formula cannot.
    ...
    #
    # Our parameterization uses a centered, scaled log_freq feature:
    #     log_freq_norm = (log f - 10) / 5
    #     L1 += l1_base_offset + l1_freq_coef * log_freq_norm
    # Reichle 2003 uses:
    #     L1 = alpha1_R - alpha2_R * log f
    # Matching the linear and constant parts gives:
    #     alpha1_R = l1_base_offset - 2 * l1_freq_coef
    #     alpha2_R = -l1_freq_coef / 5
    # alpha3 is already in Reichle's units (ms per unit cloze) since we
    # feed p = exp(-surprisal) directly into the - alpha3 * p term.
    ...
    def forward(self, word_lists, frequencies, word_lengths):
        ...
        Returns the cascade result dict plus base_L1, base_L1_formula,
        ctx, surprisal, predictability, and the Reichle L1 coefficients.
        """
