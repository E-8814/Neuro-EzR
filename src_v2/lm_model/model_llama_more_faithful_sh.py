"""
Neural EZ Reader — More Faithful variant with learned skip head.

Changes from model_llama_faithful_sh.py:
  - Removed dead `predictability` input (skip head uses LLaMA repr only)
  - Regression constants are now learnable nn.Parameters
  - L1 hard clamp raised from 1ms to 60ms (replaces soft lower-bound reg)

Core EZ Reader cascade (unchanged):
...
Integration failure (Reichle et al. 2009):
...
Learnable parameters:
  ...
  Cognitive: delta (L2/L1 ratio), l1_scale, regression_sharpness,
             regression_threshold, regression_cost_scale
"""
...
# --------------------------------------------------------------------------- #
#  Faithful Differentiable EZ Reader — learnable regression parameters
# --------------------------------------------------------------------------- #

class FaithfulEZReader(nn.Module):
    """
    Maps (L1, L2, skip_prob) → reading metrics using EZ Reader equations.
    ...
    Integration failure approximation (Reichle et al. 2009, Section 4):
      ...
      integration failure. Approximated with a sigmoid on L2.

    Regression parameters are learnable so the data can determine
    how much integration failure matters.
    """

    def __init__(self):
        super().__init__()
        self.regression_sharpness = nn.Parameter(torch.tensor(0.03))
        self.regression_threshold = nn.Parameter(torch.tensor(100.0))
        self.regression_cost_scale = nn.Parameter(torch.tensor(0.25))
...
class NeuralEZReaderLLaMA(nn.Module):
    ...
    Learnable: LLaMA top layers, projection, L1 head, skip head,
               delta, l1_scale, regression parameters.
    """
    ...
    def __init__(
        ...
        # --- Faithful EZ Reader (learnable regression parameters) ---
