"""
Neural EZ Reader Hybrid v4c_saccade_noise — v4c + Reichle's Mψ stage.

This file is v4c with the Mψ (saccade execution) stage of Reichle 2003
added to the cascade. v4c assumed deterministic landing at word center
(distance = 0 in the eccentricity formula). Reichle's Mψ instead lands
the eye at a random position determined by:

    systematic_error = (7 - intended_saccade_length) *
                       (omega1 - log(launch_duration)) / omega2
    sigma_landing   = eta1 + eta2 * intended_saccade_length
    actual_landing ~ N(target + systematic_error, sigma_landing^2)

with Reichle 2003 defaults omega1=6, omega2=3, eta1=0.5, eta2=0.15
and OPTIMAL_SACCADE_LENGTH = 7 chars.

To keep the model differentiable and closed-form, we replace stochastic
sampling with the analytic expectation of L1 over the landing
distribution. For a Gaussian distance d ~ N(mu_d, sigma^2) and the
Reichle eccentricity term L1_ecc = base_L1 * eps^(d + (w-1)/2):

    E[L1_ecc] = base_L1 * eps^(mu_d + (w-1)/2) *
                          exp(0.5 * sigma^2 * (ln eps)^2)

The first factor is what v4c had (with mu_d = 0, no distance term).
The second factor is the new variance term: as sigma grows, expected
L1 grows. Setting omega/eta to zero recovers v4c exactly.

Inputs to Mψ are oculomotor only (word_lengths, prev predicted FFD as
launch_duration). No LLaMA in Mψ; it stays a 4-parameter scalar stage.

Other than the Mψ insertion, the cascade is identical to v4c:
ctx_head, skip_residual_head, length-driven refix probability, and the
linear `pF * reg_weight * prev_gaze` regression term are unchanged.
...
# --------------------------------------------------------------------------- #
#  Reichle EZ Reader cascade with Mψ (saccade execution noise)
# --------------------------------------------------------------------------- #

class ReichleEZReaderSaccadeNoise(nn.Module):
    """
    v4c cascade + Mψ landing-error stage. Mψ adjusts the eccentricity
    factor in L1 via the closed-form expectation over the Gaussian
    landing distribution.

    New parameters (Reichle 2003 defaults):
        omega1 = 6,  omega2 = 3   (systematic error)
        eta1   = 0.5, eta2  = 0.15 (random error)
    """
    ...
    def _intended_saccade_lengths(self, word_lengths):
        ...
        For the first word in a sentence we set length = 0, which makes
        Mψ a no-op there (sys_err = 0 because (7 - 0) * ... is finite
        but masked, see _msi_correction).
        """
        prev_w = torch.cat(
        ...
        # First word: no previous saccade.
    ...
    def _msi_correction(self, intended_len, launch_dur):
        ...
        For the first word (intended_len = 0), we mask sys_err and sigma
        to zero so Mψ becomes a no-op there.
        """
    ...
    def forward(self, base_L1, L2, residual_skip_logit, word_lengths):
        ...
        # of the previous fixation. We approximate this with the v4c-style
        # FFD without any Mψ correction: L1_ecc(no Mψ) + M1 + M2.
        ...
        # --- Step 4: refixation, gaze, TRT (same as v4c) ---
        ...
        # --- Step 5: skip race (parafoveal L1 of the next word) ---
        # The parafoveal eccentricity uses a deterministic distance
        # (center of n to start of n+1). Mψ-on-saccade-into-(n+1) does
        # not affect this distance — Mψ affects where you land on n+1,
        # not where you fixate while still on n. So skip race is
        # unaffected by Mψ here.
...
class NeuralEZReaderHybrid(nn.Module):
    ...
    def __init__(
        ...
        self.ezreader = ReichleEZReaderSaccadeNoise()
