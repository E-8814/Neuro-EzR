"""
Neural EZ Reader Hybrid v4c_v3_surp — v4c_v2_surp without the
first-word skip clamp.

This is the H3 architectural ablation (ctx head replaced by
α3 · TinyLlama surprisal) brought up to the v3 skip handling, so that
the exp07 comparison against v4c_v3_dualctx is apples-to-apples:
  - FIRST_WORD_SKIP_FLOOR clamp removed (the trainer excludes
    sentence-initial words from skip supervision/eval instead);
  - skip supervision row alignment is a trainer-level choice
    (--skip_align same|next), exactly as in
    train_hybrid_v4c_v3_dualctx_geco.py.

Everything else is identical to model_llama_hybrid_v4c_v2_surp:
  - No ctx_head; base_L1 = α1 + α2·log_freq_norm + α3·surprisal
  - skip_residual_head retained
  - forward(word_lists, frequencies, word_lengths, surprisals)
"""
...
# --------------------------------------------------------------------------- #
#  Reichle EZ Reader cascade (v4c_v2 minus the first-word clamp)
# --------------------------------------------------------------------------- #


class ReichleEZReader(nn.Module):
    """v4c_v2 cascade without the first-word skip floor; M2 = I tied."""
...
class NeuralEZReaderHybrid(nn.Module):
    """
    Same as v4c_v2_surp's NeuralEZReaderHybrid; only the cascade differs
    (no first-word clamp).
    ...
    def __init__(
        ...
        # alpha3: predictability coefficient (see v4c_v2_surp notes).
    ...
    def forward(self, word_lists, frequencies, word_lengths, surprisals):
        ...
        Args:
            surprisals: Tensor of shape (batch, seq_len), per-word surprisal
                from TinyLlama. Must be precomputed (see precompute_surprisal.py).
        """
        ...
        # Predictability term: + α3 · surprisal (high surprise → larger L1
        # → slower processing); see v4c_v2_surp for sign discussion.
        base_L1_formula = (
            self.l1_base_offset
            + self.l1_freq_coef * log_freq_norm
            + self.alpha3 * surprisals
        )
