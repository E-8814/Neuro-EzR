"""
Training for model_llama_reichle_v2 on GECO corpus.

Identical to train_reichle_geco.py except:
  - imports NeuralEZReaderReichle from model_llama_reichle_v2
  - cognitive parameter prefix list uses ezreader._epsilon_raw (renamed
    from ezreader.epsilon in v1 because epsilon is now a @property
    derived from _epsilon_raw)
  - save_dir and log_path point at reichle_v2 so v1 artifacts are not
    overwritten

Usage:
  CUDA_VISIBLE_DEVICES=1 python3 -u src_v2/lm_train/train_reichle_v2_geco.py
  CUDA_VISIBLE_DEVICES=1 python3 -u src_v2/lm_train/train_reichle_v2_geco.py \
...
from model_llama_reichle_v2 import NeuralEZReaderReichle
...
def train(
    ...
    save_dir="../../checkpoints/reichle_v2/geco_tinyllama",
    log_path="../../logs/train_reichle_v2_geco.log",
    seed=42,
    ...
    # Cognitive parameter names (v2: epsilon is derived from _epsilon_raw).
    cog_name_prefixes = (
    ...
    print("\n" + "=" * 90)
    print(f"Training REICHLE v2 model on GECO")
    print(f"  Model: {model_name}")
    print(f"  v2 fixes: epsilon >= 1 (softplus reparam) + small-init residual head")
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "reichle_v2", f"geco_{model_short}"
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs", "train_reichle_v2_geco.log"
    )
