"""
Train hybrid v4c_v2 on GECO with WIDE skip prior bounds (Option C).

Same model as v4c_v2. Same training recipe except for the skip prior:

  v4c_v2 (data-anchored, narrow): bounds = data_mean ± 0.03 ≈ [0.50, 0.56],
    LAMBDA_PRIOR = 30. Penalty active most of the time, compresses skip
    prediction variance, drove r_skip from 0.55 (v4c) → 0.33.

  v4c_v2_wide_prior (this file): bounds = [0.35, 0.55] (v4c's original),
    LAMBDA_PRIOR = 30 (kept from v4c_v2). Wide bounds give the model
    freedom to produce high-variance skip predictions; LAMBDA=30 still
    catches really bad over-skipping (e.g., mean_skip > 0.55).

Hypothesis: this recovers r_skip toward 0.50 without giving up r_TRT/r_FFD
gains from v4c_v2.

All other v4c_v2 fixes preserved:
  - TRT = Gaze + I (no dead regression term)
  - First-word skip mask (skip_prob[:, 0] = 1e-6)
  ...
  - SIGMA2_FFD = 1500 (doubled FFD loss weight)
"""
...
# WIDE bounds (Option C): v4c's original range, gives the skip mechanism
# freedom to produce high-variance per-word predictions while still
# catching pathological over-skipping (mean_skip > 0.55).
SKIP_MIN = 0.35
SKIP_MAX = 0.55
...
SIGMA2_TRT = 10000.0
SIGMA2_FFD = 1500.0  # doubled FFD weight (v4c_v2 fix)
SIGMA2_GAZE = 4500.0
...
def evaluate_detailed(model, agg_data, device, subtlex, batch_size=8):
    ...
    return {
        ...
        'std_skip': float(np.std(all_pred_skip)),
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v4c_v2_wide_prior/geco_tinyllama",
    log_path="../../logs/train_hybrid_v4c_v2_wide_prior_geco.log",
    seed=42,
    ...
    # For logging/diagnostic only — bounds are NOT data-anchored in this variant.
    all_train_skips = [
        1.0 if s else 0.0
        for sd in train_raw
    ...
    print(f"  Empirical skip rate (train): {data_mean_skip:.4f} (FYI only; bounds are hardcoded wide)")
    print(f"  Skip prior bounds: [{SKIP_MIN}, {SKIP_MAX}] (LAMBDA_PRIOR={LAMBDA_PRIOR}) — WIDE")
    ...
    print(f"  Trainable cognitive:   {n_cog_trainable}    (lr={cog_lr})  (no pF/reg_weight)")
    ...
    print("\n" + "=" * 100)
    print(f"Training HYBRID v4c_v2_wide_prior on GECO  (Option C: wide skip bounds [0.35, 0.55] + LAMBDA=30)")
    ...
    print(f"  Skip prior: LAMBDA={LAMBDA_PRIOR} on bounds [{SKIP_MIN}, {SKIP_MAX}] (WIDE — Option C)")
    print(f"  SIGMA2_FFD = {SIGMA2_FFD} (doubled vs v4c, gives FFD 2x loss weight)")
    ...
    if test_agg:
        ...
        print(f"  std_skip = {test['std_skip']:.3f}")
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "hybrid_v4c_v2_wide_prior",
        f"geco_{model_short}_seed{args.seed}",
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs",
        f"train_hybrid_v4c_v2_wide_prior_geco_seed{args.seed}.log",
    )
