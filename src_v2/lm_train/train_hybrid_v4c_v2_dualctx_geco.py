"""
Train v4c_v2_dualctx on GECO.

Same training recipe as v4c_v2_wide_prior:
...
Only difference: model class is `model_llama_hybrid_v4c_v2_dualctx`,
which has two specialized ctx_heads (one for L1_FFD, one for the
parafoveal-preview L1_skip).
"""
...
def train(data_dir, num_epochs, lm_lr, head_lr, cog_lr,
    ...
    print("\n" + "=" * 100)
    print(f"v4c_v2_dualctx (two specialized ctx heads, decouples FFD↔skip)")
...
if __name__ == "__main__":
    ...
    log_path = os.path.join(
        ...
        f"train_hybrid_v4c_v2_dualctx_geco_seed{args.seed}.log",
    )
