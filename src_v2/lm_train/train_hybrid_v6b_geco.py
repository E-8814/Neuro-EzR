"""
Train the hybrid v6b Neural EZ Reader on GECO.

Same recipe as train_hybrid_v6_geco.py, with three paper-hygiene changes:

  (1) BOS sanity-check print before training. Confirms the first
      content word's surprisal is not contaminated by the BOS token's
      zero-dummy log-probability.

  (2) ctx / formula magnitude monitoring every epoch. Tracks
        mean_abs_ctx / mean_abs_formula
      so the scientific claim "formula dominates, ctx is residual" can
      be verified live. If the ratio climbs above ~0.3, the claim is
      in trouble.

  (3) --no_ctx CLI flag, passed through to the model. With --no_ctx,
      ctx_head is zeroed and frozen so training runs pure Reichle +
      LLaMA-surprisal. This is the Claim A baseline.

Save paths include a `_no_ctx` suffix when --no_ctx is set, so the two
...
from model_llama_hybrid_v6b import NeuralEZReaderHybrid
...
def bos_sanity_check(model, example_tokens):
    """
    Confirm the first content word's surprisal is not contaminated by the
    BOS position's zero-dummy log-probability.

    If tokenizer(is_split_into_words=True) prepends BOS and word_ids()
    returns None for that position, the word_map skips subword index 0
    and the first word's surprisal starts at subword 1 with a real
    log-probability — correct. If word_ids()[0] == 0 instead of None,
    the first word includes the zero dummy and its surprisal is
    under-estimated.
    """
    enc = model.tokenizer(
        [example_tokens], is_split_into_words=True, return_tensors="pt"
    )
    word_ids = enc.word_ids(batch_index=0)
    input_ids = enc["input_ids"][0].tolist()

    print(f"  BOS sanity check on: {example_tokens[:6]}...")
    print(f"    First 6 input_ids:   {input_ids[:6]}")
    print(f"    First 6 word_ids:    {word_ids[:6]}")
    if word_ids[0] is None:
        print(f"    BOS is at pos 0 with word_id=None (skipped from word_map).")
        print(f"    First word starts at subword {word_ids.index(0)}; surprisal is clean.")
    else:
        print(f"    WARNING: word_ids[0] is not None. The first word's surprisal")
        print(f"             includes the zero-dummy log-prob and is under-estimated.")
...
def evaluate_detailed(model, agg_data, device, subtlex, batch_size=8):
    ...
    all_race_logit = []
    all_surprisal, all_predictability = [], []
    all_ctx, all_formula = [], []
    ...
    with torch.no_grad():
        for i in range(0, len(agg_data), batch_size):
            ...
            for b in range(len(batch)):
                ...
                all_predictability.extend(pred['word_predictability'][b, :seq_len].cpu().tolist())
    ...
    ctx_arr = np.array(all_ctx)
    formula_arr = np.array(all_formula)
    mean_abs_ctx = float(np.mean(np.abs(ctx_arr)))
    mean_abs_formula = float(np.mean(np.abs(formula_arr)))
    ratio = mean_abs_ctx / (mean_abs_formula + 1e-8)

    # Key Claim-A/Claim-B diagnostic: is ctx silently encoding surprisal?
    r_ctx_surp = corr(all_ctx, all_surprisal)
    r_ctx_pred = corr(all_ctx, all_predictability)
    ...
    return {
        ...
        'mean_predictability': np.mean(all_predictability),
        'std_predictability': np.std(all_predictability),
        'mean_ctx': np.mean(all_ctx),
        'std_ctx': np.std(all_ctx),
        'mean_abs_ctx': mean_abs_ctx,
        'mean_abs_formula': mean_abs_formula,
        'ctx_formula_ratio': ratio,
        'r_ctx_surp': r_ctx_surp,
        'r_ctx_pred': r_ctx_pred,
    }
...
def print_cog_params(model):
    ...
    print(f"    l1_pred_coef     (alpha3)      = {model.l1_pred_coef.item():7.2f} ms  (Reichle: 39)")
    ...
    print(f"    alpha3_reichle               = {model.alpha3_reichle.item():7.2f} ms  (Reichle 2003: 39)")
    print(f"    delta (L2/L1)                = {model.delta.item():7.4f}     (Reichle: 0.34, allowed [{DELTA_MIN}, {DELTA_MAX}])")
    print(f"    epsilon (ecc exp)            = {ezr.epsilon.item():7.4f}     (Reichle: 1.15, constrained >= 1)")
    print(f"    M1 (labile)                  = {ezr.M1.item():7.2f} ms  (Reichle: 125; drives skip race)")
    print(f"    M2 = I (tied)                = {ezr.M2.item():7.2f} ms  (Reichle: M2=25, I=25)")
    print(f"    pF (integ. failure)          = {ezr.pF.item():.4f}   (Reichle: ~0.01)")
    print(f"    reg_weight                   = {ezr.reg_weight.item():.4f}")
    ...
    print(f"    no_ctx (ablation)            = {model.no_ctx}")
...
def print_sample_predictions(model, agg_data, device, subtlex, n_sentences=3, n_words=8):
    ...
    with torch.no_grad():
        for s_idx in range(min(n_sentences, len(agg_data))):
            ...
            print(f"  {'word':<14s} {'bL1':>5s} {'L1':>5s} {'L2':>5s} {'surp':>5s} {'p':>5s} {'form':>6s} {'ctx':>6s} | "
                  f"{'cTRT':>5s} {'hTRT':>5s} {'err':>5s} | "
                  f"{'pFFD':>5s} {'hFFD':>5s} | "
                  f"{'pGaze':>5s} {'hGaze':>5s} | "
                  f"{'race':>5s} {'skip':>5s} {'hSkip':>5s}")
            print(f"  {'-' * 160}")
            ...
            for i in range(min(n_words, len(s.tokens))):
                ...
                l1 = p['L1'][0, i].item()
                l2 = p['L2'][0, i].item()
                surp = p['word_surprisal'][0, i].item()
                pred_p = p['word_predictability'][0, i].item()
                formula = p['base_L1_formula'][0, i].item()
                ctx_val = p['ctx'][0, i].item()
                ...
                err = ct - ht
                print(
                    f"  {s.tokens[i]:<14s} {bl1:5.0f} {l1:5.0f} {l2:5.0f} {surp:5.2f} {pred_p:5.3f} {formula:+6.1f} {ctx_val:+6.1f} | "
                    f"{ct:5.0f} {ht:5.0f} {err:+5.0f} | "
                    f"{pf:5.0f} {hf:5.0f} | "
                    f"{pg:5.0f} {hg:5.0f} | "
                    f"{rl:+5.2f} {ps:5.2f} {hs:5.2f}"
                )
            print()
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v6b/geco_tinyllama",
    log_path="../../logs/train_hybrid_v6b_geco.log",
    seed=42,
    ...
    print(f"  no_ctx (ablation mode): {no_ctx}")
    ...
    # BOS sanity check before training.
    if train_agg:
        bos_sanity_check(model, train_agg[0].tokens)

    cog_name_prefixes = (
        "_delta_raw",
        "l1_base_offset", "l1_freq_coef", "l1_pred_coef",
    ...
    print(f"  Frozen (LM+maybe ctx): {n_frozen:,}")
    ...
    print("\n" + "=" * 100)
    print(f"Training HYBRID v6b model on GECO  (no_ctx={no_ctx})")
    print(f"  Model: {model_name}")
    print(f"  base_L1 = alpha1 + alpha2 * (log(freq)-10)/5")
    print(f"                   - alpha3 * exp(-surprisal_LLaMA)")
    if no_ctx:
        print(f"                   + 0                              (ctx DISABLED)")
    else:
        print(f"                   + ctx_head(LLaMA_hidden)")
    print(f"  base_L1 = 5 + softplus(base_L1 - 5)  (soft floor, no hard ceiling)")
    print(f"  L2 = delta * base_L1 | FFD = L1_ecc + M1 + M2")
    print(f"  Gaze = FFD + P_refix * (L2 + M1 + M2)")
    print(f"  TRT = Gaze + I + pF * reg_weight * prev_gaze    (M2 = I tied)")
    print(f"  Skip = sigmoid((M1 - L1_next_parafoveal) / skip_temperature)  (pure race, no residual)")
    print(f"  Loss: trt/SIGMA2 + ffd/SIGMA2 + gaze/SIGMA2 + bce")
    print(f"  Skip prior: hinge {LAMBDA_PRIOR}*[relu(skip-{SKIP_MAX})+relu({SKIP_MIN}-skip)]")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f"  LLaMA: surp={val['mean_surprisal']:.2f}+/-{val['std_surprisal']:.2f}  "
              f"p={val['mean_predictability']:.3f}+/-{val['std_predictability']:.3f}  "
              f"ctx={val['mean_ctx']:+.2f}+/-{val['std_ctx']:.2f} ms")
        # Claim-A/Claim-B diagnostic line.
        print(f"  Decomp: |ctx|={val['mean_abs_ctx']:.2f}ms  |form|={val['mean_abs_formula']:.2f}ms  "
              f"ratio={val['ctx_formula_ratio']:.3f}  "
              f"r(ctx,surp)={val['r_ctx_surp']:+.3f}  "
              f"r(ctx,p)={val['r_ctx_pred']:+.3f}")
        print(f"  Skip:  race_logit={val['mean_race_logit']:+.2f}+/-{val['std_race_logit']:.2f}")
        print(f"  Cog: d={model.delta.item():.3f} "
              f"a1R={model.alpha1_reichle.item():.1f} "
              f"a2R={model.alpha2_reichle.item():.3f} "
              f"a3={model.l1_pred_coef.item():.2f} "
              f"eps={model.ezreader.epsilon.item():.3f} "
              f"M1={model.ezreader.M1.item():.1f} M2=I={model.ezreader.M2.item():.1f} "
              f"pF={model.ezreader.pF.item():.4f} "
              f"reg={model.ezreader.reg_weight.item():.3f} "
              f"tau={model.ezreader.skip_temperature.item():.1f}")
    ...
    if test_agg:
        ...
        print(f"  Decomp: |ctx|={test['mean_abs_ctx']:.2f}ms  |form|={test['mean_abs_formula']:.2f}ms  "
              f"ratio={test['ctx_formula_ratio']:.3f}  "
              f"r(ctx,surp)={test['r_ctx_surp']:+.3f}  "
              f"r(ctx,p)={test['r_ctx_pred']:+.3f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no_ctx", action="store_true",
                        help="Ablate ctx_head: run pure Reichle + LLaMA-surprisal.")
    ...
    suffix = "_no_ctx" if args.no_ctx else ""
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints",
        f"hybrid_v6b{suffix}", f"geco_{model_short}"
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs",
        f"train_hybrid_v6b{suffix}_geco.log"
    )
