"""
Training for LLaMA + Differentiable EZ Reader v2 with delta constraint on GECO corpus.

This is model_llama_v2_delta.py: same DiffEZReader v2 as model_llama.py, but
L2 = delta * L1 instead of two independent heads. This eliminates the need for
L2 lower-bound regularizers since L2 is structurally bounded.

Key differences from train_geco_llama.py (v2):
  - Import model_llama_v2_delta (L2 = delta * L1)
  - No L2 floor regularizer (structurally bounded)
  - _delta_raw grouped with EZR params for higher learning rate

Key differences from train_geco_llama_v3.py:
  - Uses DiffEZReader v2 (TRT = (1-skip) * reading_time), not v3
  - Loss on all words (no skip-masking)

Usage:
  python3 -u src_v2/train_geco_llama_v2_delta.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
  python3 -u src_v2/train_geco_llama_v2_delta.py --ablation no_eccentricity
"""
...
# --------------------------------------------------------------------------- #
#  Loss function (v2 style: all words, no skip-masking)
# --------------------------------------------------------------------------- #
...
def compute_loss(pred, human_trt, human_ffd, human_gaze, human_skip):
    ...
    Same as v2 loss but without L2 lower bound regularizer
    (L2 = delta * L1 is structurally bounded).
    """
...
def train(
    ...
    save_dir="../../checkpoints/v2_delta/geco_llama",
    seed=42,
    ...
    print(f"Training v2-delta (LLaMA + DiffEZR v2, L2=delta*L1) on GECO{ablation_label}")
    print(f"  Model: {model_name}")
    print(f"  v2-delta: L2 = delta * L1 (single head, theory-constrained) + DiffEZR v2")
...
if __name__ == "__main__":
    ...
    if args.ablation:
        save_dir = os.path.join(os.path.dirname(__file__), "..", "..",
                                "checkpoints", "v2_delta", f"geco_{model_short}_ablation_{args.ablation}")
    else:
        save_dir = os.path.join(os.path.dirname(__file__), "..", "..", "checkpoints", "v2_delta", f"geco_{model_short}")
