"""
Train v4c_v2_wide_prior with STAGED training (Option G).

Phase 1 (epochs 1..freeze_skip_after): train everything normally.
Phase 2 (epochs freeze_skip_after+1..end): FREEZE skip_residual_head;
    continue training rest (LLaMA, projection, ctx_head, cog scalars).

Hypothesis: skip_residual_head reaches its peak r_skip early (val 3-4)
then degrades (residual_abs grows past optimum). Freezing it after the
early peak preserves its near-best state while ctx_head + L1 continue
to develop, letting r_FFD catch up.

CLI: --freeze_skip_after_epoch N (default 1)
    Skip residual head is trained during epoch 1, then frozen at the
    start of epoch 2 onward.

Everything else identical to wide_prior:
  - WIDE skip prior bounds [0.35, 0.55], LAMBDA_PRIOR = 30
  - LAMBDA_SKIP_RESIDUAL = 0.001 (no extra L2 reg; that was Option E)
...
def freeze_skip_residual(model):
    """Set requires_grad=False on all skip_residual_head params."""
    n = 0
...
def train(data_dir, num_epochs, lm_lr, head_lr, cog_lr,
          ...
          seed, model_name, freeze_layers, freeze_skip_after_epoch):
    ...
    print(f"  Skip residual head will be FROZEN starting at epoch {freeze_skip_after_epoch + 1}")
    ...
    print("\n" + "=" * 100)
    print(f"v4c_v2_wide_prior_staged (Option G)")
    print(f"  Phase 1 (epochs 1..{freeze_skip_after_epoch}): train everything")
    print(f"  Phase 2 (epochs {freeze_skip_after_epoch+1}+): freeze skip_residual_head")
    print("=" * 100)

    for epoch in range(1, num_epochs + 1):
        if early_stop: break

        # --- Phase transition: freeze skip residual at start of phase 2 ---
        if epoch == freeze_skip_after_epoch + 1 and not skip_frozen:
            print(f"\n>>> [Epoch {epoch}] Phase transition: freezing skip_residual_head")
        ...
        for step in range(n_batches):
            ...
            if (step + 1) % val_every == 0 or is_last:
                ...
                phase = "1" if not skip_frozen else "2 (skip frozen)"
                print(f"\n  [val {total_val_steps}] epoch {epoch} batch {step+1}/{n_batches} | phase {phase}")
    ...
    if test_agg:
        ...
        if os.path.exists(ckpt_path):
            ...
            print(f"Loaded best checkpoint (epoch {ckpt['epoch']}, val {ckpt['val_step']}, "
                  f"skip_frozen={ckpt.get('skip_residual_frozen', '?')})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--freeze_skip_after_epoch", type=int, default=1,
                        help="Freeze skip_residual_head starting at epoch N+1.")
    ...
    save_dir = os.path.join(
        ...
        "hybrid_v4c_v2_wide_prior_staged",
        f"geco_{model_short}_seed{args.seed}_freeze{args.freeze_skip_after_epoch}",
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs",
        f"train_hybrid_v4c_v2_wide_prior_staged_geco_seed{args.seed}_freeze{args.freeze_skip_after_epoch}.log",
    )
    ...
    train(
        ...
        freeze_skip_after_epoch=args.freeze_skip_after_epoch,
    )
