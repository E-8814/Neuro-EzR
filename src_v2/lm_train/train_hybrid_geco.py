"""
Train the hybrid Neural EZ Reader on GECO.

Same training recipe as train_reichle_v3_geco.py with the minimum changes
required by the hybrid architecture:
  - no SUBTLEX frequency lookup (hybrid doesn't take frequencies as input)
  - no residual regularizer (hybrid has no residual head)
  - no alpha1/alpha2/alpha3 logging (hybrid has no formula coefficients)
  - no surprisal logging (hybrid does not compute per-word surprisal)
  - forward call is model(word_lists, word_lengths) instead of
    model(word_lists, frequencies, word_lengths)

Loss, optimizer, scheduler, skip prior, delta regularizer, and all
training hyperparameters are unchanged from train_reichle_v3_geco.py.
"""
...
from model_llama_hybrid import NeuralEZReaderHybrid
...
def train(
    ...
    save_dir="../../checkpoints/hybrid/geco_tinyllama",
    log_path="../../logs/train_hybrid_geco.log",
    seed=42,
    ...
    print("\n" + "=" * 90)
    print(f"Training HYBRID model on GECO")
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "hybrid", f"geco_{model_short}"
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs", "train_hybrid_geco.log"
    )
