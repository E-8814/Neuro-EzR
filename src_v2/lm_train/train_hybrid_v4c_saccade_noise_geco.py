"""
Train hybrid v4c_saccade_noise on GECO.

Same recipe as train_hybrid_v4c_geco.py with the following adjustments
to handle the 4 new Mψ parameters (omega1, omega2, eta1, eta2):

  - cog_name_prefixes adds the 4 Mψ raw scalars.
  - print_cog_params reports omega1/omega2/eta1/eta2 alongside the
    existing parameters.
  - Per-epoch log line includes mean sys_err and mean sigma_landing
    (sanity-check that Mψ is doing something).
  - Save paths bumped from hybrid_v4c to hybrid_v4c_saccade_noise.
"""
...
from model_llama_hybrid_v4c_saccade_noise import NeuralEZReaderHybrid
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v4c_saccade_noise/geco_tinyllama",
    log_path="../../logs/train_hybrid_v4c_saccade_noise_geco.log",
    seed=42,
    ...
    print(f"  Trainable cognitive:   {n_cog_trainable}    (lr={cog_lr})  (+4 Mψ params vs v4c)")
    ...
    print("\n" + "=" * 100)
    print(f"Training HYBRID v4c_saccade_noise on GECO  (v4c + Mψ saccade execution noise)")
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        ...
        "hybrid_v4c_saccade_noise",
        f"geco_{model_short}_seed{args.seed}",
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs",
        f"train_hybrid_v4c_saccade_noise_geco_seed{args.seed}.log",
    )
