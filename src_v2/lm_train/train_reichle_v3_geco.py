"""
Training for model_llama_reichle_v3 on GECO corpus.

Identical to train_reichle_v2_geco.py except:
  - imports NeuralEZReaderReichle from model_llama_reichle_v3
  - DELTA_MIN lowered from 0.20 to 0.10 so the regularizer allows the
    data-preferred delta (around 0.14 in v2 training) without fighting it
  - save_dir and log_path point at reichle_v3

Usage:
  CUDA_VISIBLE_DEVICES=1 python3 -u src_v2/lm_train/train_reichle_v3_geco.py
  CUDA_VISIBLE_DEVICES=1 python3 -u src_v2/lm_train/train_reichle_v3_geco.py \
...
from model_llama_reichle_v3 import NeuralEZReaderReichle
...
SKIP_TARGET = 0.45
DELTA_MIN = 0.10   # v3: lowered from 0.20 so data-preferred delta is legal
DELTA_MAX = 0.50
...
def print_cog_params(model):
    ...
    print(f"    alpha3 (surprisal coef)= {model.alpha3.item():7.4f}    (init 4.0 ms/nat, clipped @ 12)")
    print(f"    delta  (L2/L1)         = {model.delta.item():7.4f}    (Reichle 2003: 0.34, allowed >= {DELTA_MIN})")
...
def train(
    ...
    save_dir="../../checkpoints/reichle_v3/geco_tinyllama",
    log_path="../../logs/train_reichle_v3_geco.log",
    seed=42,
    ...
    print("\n" + "=" * 90)
    print(f"Training REICHLE v3 model on GECO")
    print(f"  Model: {model_name}")
    print(f"  v3 fixes: L1 floor -> 30 ms, surprisal clipped at 12 nats, "
          f"delta allowed >= {DELTA_MIN}")
    print(f"  L1 = alpha1 - alpha2*ln(freq) + alpha3*clip(surprisal, 12) + residual_LLM")
    ...
    print(f"  AMP: {use_amp}")
    print(f"  Regularizers: delta_reg({DELTA_MIN}-{DELTA_MAX}) + "
          f"skip_prior({SKIP_TARGET}) + residual_L2({LAMBDA_RESIDUAL})")
    print("=" * 90)
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "reichle_v3", f"geco_{model_short}"
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs", "train_reichle_v3_geco.log"
    )
