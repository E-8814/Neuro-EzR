"""
Train the hybrid v6 Neural EZ Reader on GECO.

Same recipe as train_hybrid_v5_geco.py (loss-scale normalization, hinge
skip prior, cosine LR with warmup, early stopping on r_TRT) with the
minimum changes required by model_llama_hybrid_v6.NeuralEZReaderHybrid:

  - Skip is produced by the parafoveal race alone (no residual head).
    The skip-residual L2 penalty is removed from the loss.
  - print_cog_params / per-epoch log reports alpha1_reichle and
    alpha2_reichle (Reichle-unit equivalents of l1_base_offset and
    l1_freq_coef) alongside the normalized parameters.
"""
...
from model_llama_hybrid_v6 import NeuralEZReaderHybrid
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v6/geco_tinyllama",
    log_path="../../logs/train_hybrid_v6_geco.log",
    seed=42,
    ...
    print("\n" + "=" * 100)
    print(f"Training HYBRID v6 model on GECO")
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "hybrid_v6", f"geco_{model_short}"
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs", "train_hybrid_v6_geco.log"
    )
