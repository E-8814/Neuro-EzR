"""
Train the hybrid v4b Neural EZ Reader on GECO.

v4b model differences from v4: l1_neural_head replaced by explicit
alpha3 * exp(-surprisal) term; M2 and I tied; AutoModelForCausalLM
backbone. Skip mechanism (race + residual) kept — v6b showed the
residual is structurally necessary.

Training recipe is v4's, with the minimum adjustments required:

  - Import v4b model; LM parameter group filter matches "lm." prefix.
  - cog_name_prefixes: drops ezreader._M2_raw / ezreader._I_raw (tied),
    adds ezreader._M2I_raw and l1_pred_coef.
  - print_cog_params / log line reports alpha1_reichle, alpha2_reichle,
    alpha3_reichle directly.
  - Checkpoint saves the Reichle-unit alpha values alongside raw ones.

Skip residual L2 regularizer kept at v4's LAMBDA_SKIP_RESIDUAL = 0.001.
"""
...
from model_llama_hybrid_v4b import NeuralEZReaderHybrid
...
def print_sample_predictions(model, agg_data, device, subtlex, n_sentences=3, n_words=8):
    ...
    with torch.no_grad():
        for s_idx in range(min(n_sentences, len(agg_data))):
            ...
            print(f"  {'word':<14s} {'bL1':>5s} {'L1':>5s} {'L2':>5s} {'surp':>5s} {'p':>5s} | "
            ...
            for i in range(min(n_words, len(s.tokens))):
                ...
                err = ct - ht
                print(
                    f"  {s.tokens[i]:<14s} {bl1:5.0f} {l1:5.0f} {l2:5.0f} {surp:5.2f} {pred_p:5.3f} | "
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v4b/geco_tinyllama",
    log_path="../../logs/train_hybrid_v4b_geco.log",
    seed=42,
    ...
    print(f"  Trainable heads:       {n_head_trainable:,}   (lr={head_lr})  (projection + skip_residual_head)")
    ...
    print("\n" + "=" * 100)
    print(f"Training HYBRID v4b model on GECO  (v4 minus l1_neural_head, plus explicit alpha3)")
    print(f"  Model: {model_name}")
    print(f"  base_L1 = alpha1 + alpha2 * (log(freq)-10)/5  - alpha3 * exp(-surprisal)")
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "hybrid_v4b", f"geco_{model_short}"
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs", "train_hybrid_v4b_geco.log"
    )
