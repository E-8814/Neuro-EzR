"""
Train hybrid v4c_v2 on GECO.

Differences from train_hybrid_v4c_geco.py:

  (1) RELOAD BEST CHECKPOINT BEFORE TEST EVAL.
      v4c reported test numbers from the FINAL epoch's model state, not
      from best_model.pt. v4c_v2 explicitly reloads the best checkpoint
      before the test pass.

  (2) DATA-ANCHORED SKIP PRIOR.
      v4c hard-coded SKIP_MIN/SKIP_MAX = [0.35, 0.55]. Empirically
      mean_skip on v4c was 0.66 — outside the bound, with LAMBDA_PRIOR=20
      not strong enough to enforce. v4c_v2 computes the empirical skip
      rate from the training data and sets bounds = data_mean ± 0.03,
      with LAMBDA_PRIOR raised to 30 (combined of the two suggested
      tightenings).

  (3) COMBINED EARLY-STOPPING METRIC.
      v4c selected best epoch on r_TRT only, which gave a model with
      r_FFD = 0.118 (FFD continues to improve through epoch 3 → 0.195).
      v4c_v2 selects on combined = (r_TRT + r_FFD + r_Gaze + r_skip) / 4.

  (4) FIRST-WORD SKIP MASK (in model).
      Already in model_llama_hybrid_v4c_v2.py. skip_prob[:, 0] = 1e-6.

  (5) DROP DEAD REGRESSION TERM (in model).
      Already in model_llama_hybrid_v4c_v2.py. cog_name_prefixes here
      no longer references _pF_raw / _reg_weight_raw.

  (6) LOWER LRs FOR HEAD AND COG.
      cog_lr: 1e-3 → 3e-4 (3.3x slower)
      head_lr: 5e-4 → 2e-4 (2.5x slower)
      lm_lr: unchanged (2e-5)
      Slows the post-epoch-1 cog-param drift; trades a bit of epoch-1
      convergence for cleaner trajectories.

  (7) MID-EPOCH VALIDATION.
      Validates every n_batches // 5 steps (5 vals per epoch). With
      epoch-1 convergence behavior, this captures the actual best
      training step instead of only seeing it at epoch boundaries.
      Patience counter operates on val-steps now, not epochs.

Loss is unchanged: MSE on TRT/FFD/Gaze (sigma-normalized) + BCE on skip.
"""
...
LAMBDA_DELTA = 5.0
LAMBDA_PRIOR = 30.0  # bumped from 20 (combined with data-anchored bounds)
LAMBDA_SKIP_RESIDUAL = 0.001
SKIP_BOUND_HALFWIDTH = 0.03  # bounds = data_mean ± 0.03
DELTA_MIN = 0.10
DELTA_MAX = 0.50
...
EARLY_STOP_PATIENCE_VALS = 15  # in val-steps (with 5 vals/epoch ≈ 3 epochs)
WARMUP_EPOCHS = 2
N_VALS_PER_EPOCH = 5
...
def print_cog_params(model):
    ...
    print(f"    (pF, reg_weight DROPPED in v4c_v2 — lesion showed Δr ≈ -0.001)")
...
def print_sample_predictions(model, agg_data, device, subtlex, n_sentences=3, n_words=8):
    ...
    with torch.no_grad():
        for s_idx in range(min(n_sentences, len(agg_data))):
            ...
            print(f"  {'word':<14s} {'bL1':>5s} {'L1':>5s} {'L2':>5s} {'form':>6s} {'ctx':>6s} | "
            ...
            for i in range(min(n_words, len(s.tokens))):
                ...
                err = ct - ht
                print(
                    f"  {s.tokens[i]:<14s} {bl1:5.0f} {l1:5.0f} {l2:5.0f} {formula:+6.1f} {ctx_val:+6.1f} | "
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v4c_v2/geco_tinyllama",
    log_path="../../logs/train_hybrid_v4c_v2_geco.log",
    seed=42,
    ...
    print(f"  Trainable heads:       {n_head_trainable:,}   (lr={head_lr})  (projection + ctx_head + skip_residual_head)")
    print(f"  Trainable cognitive:   {n_cog_trainable}    (lr={cog_lr})  (no pF/reg_weight in v4c_v2)")
    ...
    print("\n" + "=" * 100)
    print(f"Training HYBRID v4c_v2 on GECO  (drop dead regression term + first-word skip mask + 5 fixes)")
    print(f"  Model: {model_name}")
    print(f"  base_L1 = alpha1 + alpha2 * (log(freq)-10)/5  + ctx_head(LLaMA_hidden)")
    print(f"  base_L1 = 5 + softplus(base_L1 - 5)  (soft floor)")
    print(f"  L2 = delta * base_L1 | FFD = L1_ecc + M1 + M2")
    print(f"  Gaze = FFD + P_refix(length) * (L2 + M1 + M2)")
    print(f"  TRT = Gaze + I            (M2 = I tied; v4c's pF*reg_weight*prev_gaze DROPPED)")
    print(f"  Skip = sigmoid((M1 - L1_next_parafoveal)/skip_temperature + residual)")
    print(f"  First-word skip forced to {model.ezreader.FIRST_WORD_SKIP_FLOOR} (anatomical mask)")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # Sample predictions only at end of each epoch.
    ...
    if test_agg:
        # --- v4c_v2 fix: reload best checkpoint before test eval ---
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "hybrid_v4c_v2",
        f"geco_{model_short}_seed{args.seed}",
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs",
        f"train_hybrid_v4c_v2_geco_seed{args.seed}.log",
    )
