"""
Train hybrid v4c_v2_full on GECO.

Stacks v4c_v2's training recipe with v4c_full's full cognitive cascade.
  ...
  v4c_full additions (cascade, in model file):
    - Mψ saccade-execution noise + closed-form expected L1.
    - Landing-error-driven refixation (replaces length-driven).
    - Untied I, M2, A (separate integration_time, saccade-finishing,
      attention-shift parameters).
    - Stochastic integration failure: p_fail = sigmoid(ifail_offset +
      ifail_coef * log_freq_norm).
    - Regression direction: p_correct (Reichle: 0.6).
    - TRT = Gaze + I + p_fail * regression_cost.

cog_name_prefixes covers:
  - alpha1/alpha2 (l1_base_offset, l1_freq_coef), delta
  - epsilon, M1, M2, I, A
  - Mψ: omega1, omega2, eta1, eta2
  - lambda_refix
  - ifail_offset, ifail_coef, p_correct
  - skip_temperature
  - NO pF or reg_weight (replaced by explicit mechanic).
"""
...
from model_llama_hybrid_v4c_v2_full import NeuralEZReaderHybrid
...
def print_cog_params(model):
    ...
    print(f"    (pF, reg_weight DROPPED — replaced by explicit p_fail mechanic)")
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v4c_v2_full/geco_tinyllama",
    log_path="../../logs/train_hybrid_v4c_v2_full_geco.log",
    seed=42,
    ...
    print("\n" + "=" * 100)
    print(f"Training HYBRID v4c_v2_full on GECO  (v4c_v2 hygiene + full Reichle cascade)")
        ...
        for step in range(n_batches):
            ...
            if (step + 1) % val_every_n == 0 or is_last_batch:
                ...
                print(f"    Cog:   a1R={model.alpha1_reichle.item():.1f} a2R={model.alpha2_reichle.item():.3f} "
                      ...
                      f"M2={model.ezreader.M2.item():.1f} I={model.ezreader.I.item():.1f} A={model.ezreader.A.item():.1f}")
                print(f"           w1={model.ezreader.omega1.item():.2f} w2={model.ezreader.omega2.item():.2f} "
                      ...
                      f"lam={model.ezreader.lambda_refix.item():.3f} pcr={model.ezreader.p_correct.item():.3f}")
    ...
    if test_agg:
        ...
        print(f"  Mψ:    sys_err={test['mean_sys_err']:+.2f}+/-{test['std_sys_err']:.2f}  "
              ...
              f"refix={test['mean_refix']:.3f}+/-{test['std_refix']:.3f}")
        print(f"  Integ: p_fail={test['mean_p_fail']:.3f}+/-{test['std_p_fail']:.3f}")
...
if __name__ == "__main__":
    ...
    log_path = os.path.join(
        ...
        f"train_hybrid_v4c_v2_full_geco_seed{args.seed}.log",
    )
