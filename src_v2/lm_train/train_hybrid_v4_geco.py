"""
Train the hybrid v4 Neural EZ Reader on GECO.

Same training recipe as train_hybrid_v2_geco.py (loss-scale
normalization, hinge skip prior, cosine LR schedule with warmup,
patience-based early stopping) with the minimum changes required by
model_llama_hybrid_v4.NeuralEZReaderHybrid:

  - Loads SUBTLEX frequencies and threads them through collate_* and
    model.forward().
  - Updated cog_name_prefixes: l1_base_offset replaces l1_scale, and
    ezreader._skip_temperature_raw is added.
  - compute_loss consumes a residual_skip_reg term so the skip residual
    head stays small while the parafoveal race carries the skip signal.
  - print_cog_params / print_sample_predictions updated for the new
    parameter set and for the race / residual breakdown in skip.

The model's forward() signature is (word_lists, frequencies,
word_lengths). Skip probability now comes from the parafoveal race
inside the cascade rather than a parallel parafoveal head.
"""
...
from model_llama_hybrid_v4 import NeuralEZReaderHybrid
...
def compute_loss(pred, human_trt, human_ffd, human_gaze, human_skip, delta):
    ...
    # Symmetric hinge skip prior.
    mean_skip = pred_skip.mean()
    skip_prior = LAMBDA_PRIOR * (
        F.relu(mean_skip - SKIP_MAX) + F.relu(SKIP_MIN - mean_skip)
    )

    # Keep the residual skip head small so the parafoveal race carries
    # the primary skip signal. L2 penalty on the residual logit.
...
def print_cog_params(model):
    ...
    print(f"    l1_base_offset       = {model.l1_base_offset.item():7.2f} ms  (alpha1 analogue, Reichle: 104)")
    print(f"    l1_freq_coef         = {model.l1_freq_coef.item():7.2f}     (alpha2 on normed log_freq; init -17)")
    ...
    print(f"    M1 (labile)          = {ezr.M1.item():7.2f} ms  (Reichle: 125; now drives skip race)")
...
def print_sample_predictions(model, agg_data, device, subtlex, n_sentences=3, n_words=8):
    ...
    with torch.no_grad():
        for s_idx in range(min(n_sentences, len(agg_data))):
            ...
            print(f"  {'word':<14s} {'bL1':>5s} {'L1':>5s} {'L2':>5s} | "
            ...
            for i in range(min(n_words, len(s.tokens))):
                ...
                err = ct - ht
                print(
                    f"  {s.tokens[i]:<14s} {bl1:5.0f} {l1:5.0f} {l2:5.0f} | "
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v4/geco_tinyllama",
    log_path="../../logs/train_hybrid_v4_geco.log",
    seed=42,
    ...
    cog_name_prefixes = (
        "_delta_raw", "l1_base_offset", "l1_freq_coef",
    ...
    print("\n" + "=" * 100)
    print(f"Training HYBRID v4 model on GECO")
    print(f"  Model: {model_name}")
    print(f"  base_L1 = l1_base_offset + l1_freq_coef * (log(freq)-10)/5 + l1_neural_head(LLaMA)")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f"  Cog: d={model.delta.item():.3f} "
              f"l1_off={model.l1_base_offset.item():.1f} "
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "hybrid_v4", f"geco_{model_short}"
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs", "train_hybrid_v4_geco.log"
    )
