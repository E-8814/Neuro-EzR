"""
Train the hybrid v3 Neural EZ Reader on GECO.

Based on train_hybrid_v2_geco.py. Two training-recipe changes driven by the
v1-vs-v2 log analysis (v2 gave skip BCE ~29x more relative weight than v1,
which pulled the shared projection toward skip features and cost ~0.01
r_Gaze). Combined with v3's architectural split projection these recover
the duration quality while keeping most of v2's skip gain.

  (i) Skip BCE weight lowered from 1.0 to 0.15. After loss-scale
      normalization the duration losses are each ~1.0; with 0.15 on skip
      BCE, the skip contribution is roughly 0.15 * 0.06 / 2.3 ~ 0.4% of
      the loss (vs 2.4% in v2 and 0.086% in v1 — halfway between, closer
      to v1). The v3 model's split projection is what actually protects
      L1 / gaze from skip gradients, so we can afford to keep a small
      skip weight without trade.

  (ii) Hinge prior strength reduced from 20 to 10. v2's hinge with 20
       had a constant gradient of 20 outside the [0.35, 0.55] band, which
       is very strong; 10 still escapes the skip-collapse failure mode
       but does not dominate the early epochs of optimization.

Everything else (loss-scale normalization, length concat on the skip
head — which is now wired into skip_projection in the model, cosine LR
with warmup, early stopping, delta regularizer) is unchanged from
train_hybrid_v2_geco.py.
"""
...
from model_llama_hybrid_v3 import NeuralEZReaderHybrid
...
LAMBDA_DELTA = 5.0
LAMBDA_PRIOR = 10.0          # v3: was 20 in v2
SKIP_LOSS_WEIGHT = 0.15      # v3: was 1.0 in v2
SKIP_MIN = 0.35
SKIP_MAX = 0.55
DELTA_MIN = 0.10
DELTA_MAX = 0.50
...
def print_cog_params(model):
    ...
    print(f"    L1 floor (soft)  = {ezr.L1_MIN} ms  L1 ceiling = {ezr.L1_MAX} ms")
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v3/geco_tinyllama",
    log_path="../../logs/train_hybrid_v3_geco.log",
    seed=42,
    ...
    print("\n" + "=" * 90)
    print(f"Training HYBRID v3 model on GECO")
    print(f"  Model: {model_name}")
    print(f"  Architecture: SPLIT PROJECTION (l1/gaze vs skip), softplus L1 floor")
    print(f"  base_L1 = l1_head(projection(LLaMA)) * l1_scale")
    print(f"  skip    = skip_head([skip_projection(LLaMA), len/10])")
    print(f"  L2 = delta * base_L1 | FFD = L1_ecc + M1 + M2")
    print(f"  Gaze = FFD + P_refix * (L2 + M1 + M2)")
    print(f"  TRT = Gaze + I + pF * reg_weight * prev_gaze")
    print(f"  Loss: trt/SIGMA2 + ffd/SIGMA2 + gaze/SIGMA2 + {SKIP_LOSS_WEIGHT}*bce")
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "hybrid_v3", f"geco_{model_short}"
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs", "train_hybrid_v3_geco.log"
    )
