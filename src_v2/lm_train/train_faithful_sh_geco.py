"""
Training for model_llama_faithful_sh on GECO corpus.

Same as train_faithful_geco.py but with a learned skip head.
The skip head is supervised with BCE loss against human skip rates.

Faithful L1→L2 cascade is unchanged:
  FFD = L1 | Gaze = L1 + L2 | TRT = Gaze + regression (conditional)
  Time losses masked to fixated words only.

Added for skip head:
  - BCE skip loss (on all words)
  - Skip prior regularizer (mean skip ≈ target)

Usage:
  python3 -u src_v2/lm_train/train_faithful_sh_geco.py
  python3 -u src_v2/lm_train/train_faithful_sh_geco.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
"""
...
def train(
    ...
    save_dir="../../checkpoints/faithful_sh/geco_llama",
    log_path="../../logs/train_faithful_sh_geco.log",
    seed=42,
    ...
    print("\n" + "=" * 90)
    print(f"Training FAITHFUL+SH model on GECO")
    ...
    print(f"  Loss: 0.25*TRT(cond) + 0.25*FFD + 0.25*Gaze + 0.4*skip_BCE")
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.path.dirname(__file__), "..", "..", "checkpoints", "faithful_sh", f"geco_{model_short}")
    log_path = os.path.join(os.path.dirname(__file__), "..", "..", "logs", f"train_faithful_sh_geco.log")
