"""
Training for model_llama_faithful on GECO corpus.

Same model and training logic as train_faithful_provo.py, just on GECO data.
...
is replacing L1 = α1 - α2·ln(freq) - α3·pred with a neural LM.
  ...
  TRT = Gaze + regression (conditional on fixation).
  No skip loss. Time losses masked to fixated words only.

Usage:
  python3 -u src_v2/lm_train/train_faithful_geco.py
  python3 -u src_v2/lm_train/train_faithful_geco.py --model TinyLlama/TinyLlama-1.1B-Chat-v1.0
"""
...
def compute_loss(pred, human_trt, human_ffd, human_gaze, human_skip, delta):
    ...
    # Mask: only fixated words (skip=0 in per-participant data)
...
def train(
    ...
    save_dir="../../checkpoints/faithful/geco_llama",
    log_path="../../logs/train_faithful_geco.log",
    seed=42,
    ...
    print("\n" + "=" * 90)
    print(f"Training FAITHFUL model (Reichle et al. equations) on GECO")
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(os.path.dirname(__file__), "..", "..", "checkpoints", "faithful", f"geco_{model_short}")
    log_path = os.path.join(os.path.dirname(__file__), "..", "..", "logs", f"train_faithful_geco.log")
