"""
Train the Insanity v2 Neural EZ Reader on GECO.

Differences vs train_insanity_geco.py:

  (1) Model is model_llama_insanity_v2.NeuralEZReaderInsanity, whose
      L1 head has an explicit Reichle-style lexical anchor. The lex
      coefficients (l1_bias, l1_freq_coef, l1_len_coef, l1_surp_coef)
      are added to the cog_params group so they get the cog_lr rate.

  (2) Sigma regularization. A new loss term penalizes
      log(sigma_ffd / gaze / trt) drifting above log(SIGMA_TARGET),
      with SIGMA_TARGET = 0.25. The penalty is a one-sided soft cap
      (ReLU((log_sigma - log_target)^2) * LAMBDA_SIGMA), so the
      heads are free to have sigma <= 0.25 but pay for widening
      beyond it. This redirects gradient pressure from "widen sigma
      to absorb residual" toward "fit the median better".

  (3) Logging shows the four lex coefficients each epoch so you can
      see whether the optimizer is updating them away from init.

Everything else (SUBTLEX, collate, eval, progress logging) is
identical to train_insanity_geco.py.
"""
...
from model_llama_insanity_v2 import NeuralEZReaderInsanity, log_normal_nll
...
# Sigma soft cap: penalize log(sigma) > log(SIGMA_TARGET).
SIGMA_TARGET = 0.25
LOG_SIGMA_TARGET = math.log(SIGMA_TARGET)
LAMBDA_SIGMA = 0.3

EARLY_STOP_PATIENCE = 10
WARMUP_EPOCHS = 2

PROGRESS_FRACTION = 20
...
# --------------------------------------------------------------------------- #
#  Loss: log-normal NLL + BCE + delta hinge + sigma soft cap
# --------------------------------------------------------------------------- #
...
def _sigma_soft_cap_penalty(sigma):
    """
    One-sided penalty: log(sigma) is free below LOG_SIGMA_TARGET, but
    pays a quadratic cost above it. Shape: 0 below target, quadratic above.
    """
    log_sigma = torch.log(sigma)
    excess = F.relu(log_sigma - LOG_SIGMA_TARGET)
...
def compute_loss(pred, human_trt, human_ffd, human_gaze, human_skip, delta):
    ...
    sigma_reg = LAMBDA_SIGMA * (
        _sigma_soft_cap_penalty(ffd_sigma)
        + _sigma_soft_cap_penalty(gaze_sigma)
        + _sigma_soft_cap_penalty(trt_sigma)
    )

    total = nll_ffd + nll_gaze + nll_trt + bce_fix + delta_reg + sigma_reg
...
def evaluate_detailed(model, agg_data, device, subtlex, batch_size=8):
    ...
    with torch.no_grad():
        for i in range(0, len(agg_data), batch_size):
            ...
            for b in range(len(batch)):
                ...
                all_l1_lex.extend(pred['l1_lex'][b, :seq_len].cpu().tolist())
                all_l1_residual.extend(pred['l1_residual'][b, :seq_len].cpu().tolist())
    ...
    return {
        ...
        'mean_l1_lex': np.mean(all_l1_lex),
        'std_l1_lex': np.std(all_l1_lex),
        'mean_l1_residual': np.mean(all_l1_residual),
        'std_l1_residual': np.std(all_l1_residual),
...
def print_cog_params(model):
    ...
    print(f"  L1 head lex coefficients:")
    print(f"    l1_bias              = {model.l1_bias.item():7.2f} ms  (init 60)")
    print(f"    l1_freq_coef         = {model.l1_freq_coef.item():7.2f}     (init -15, per unit log_freq_norm)")
    print(f"    l1_len_coef          = {model.l1_len_coef.item():7.2f}     (init   3, per unit log_len_norm)")
    print(f"    l1_surp_coef         = {model.l1_surp_coef.item():7.2f}     (init   4, per unit surprisal_norm)")
...
def print_sample_predictions(model, agg_data, device, subtlex, n_sentences=3, n_words=8):
    ...
    with torch.no_grad():
        for s_idx in range(min(n_sentences, len(agg_data))):
            ...
            print(f"  {'word':<14s} {'surp':>5s} {'lex':>5s} {'res':>5s} {'bL1':>5s} {'L1e':>5s} | "
            ...
            for i in range(min(n_words, len(s.tokens))):
                lex = p['l1_lex'][0, i].item()
                res = p['l1_residual'][0, i].item()
                ...
                err = tt - ht
                print(
                    f"  {s.tokens[i]:<14s} {su:5.1f} {lex:5.0f} {res:+5.1f} {bl1:5.0f} {l1e:5.0f} | "
...
def train(
    ...
    save_dir="../../checkpoints/insanity_v2/geco_tinyllama",
    log_path="../../logs/train_insanity_v2_geco.log",
    seed=42,
    ...
    # Parameter groups.
    # L1 lex coefficients are explicit cognitive parameters so they live
    # in cog_params under the cog_lr rate, not the head_lr rate.
    l1_lex_param_names = {"l1_bias", "l1_freq_coef", "l1_len_coef", "l1_surp_coef"}
    ...
    for name, param in model.named_parameters():
        ...
        elif name.startswith("cascade.") or name in l1_lex_param_names:
    ...
    print("\n" + "=" * 100)
    print(f"Training INSANITY v2 model on GECO")
    print(f"  Model: {model_name}")
    print(f"  L1 head: structured lex anchor + small neural residual")
    print(f"    L1_lex = l1_bias + l1_freq_coef*log_freq_norm + "
          f"l1_len_coef*log_len_norm + l1_surp_coef*surprisal_norm")
    print(f"    base_L1 = 5 + softplus(L1_lex + l1_residual(state) - 5)")
    ...
    print(f"  Loss: nll_ffd + nll_gaze + nll_trt + bce_fix + delta_reg + sigma_reg")
    print(f"  Sigma soft cap: log_target={LOG_SIGMA_TARGET:.3f} "
          f"(target={SIGMA_TARGET}), lambda={LAMBDA_SIGMA}")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        epoch_loss = 0.0
        epoch_nll_ffd = epoch_nll_gaze = epoch_nll_trt = 0.0
        epoch_bce = epoch_sigma_reg = 0.0
        n_samples = 0

        run_loss = 0.0
        run_nll_ffd = run_nll_gaze = run_nll_trt = 0.0
        run_bce = run_sigma_reg = 0.0
        run_steps = 0
        run_t0 = time.time()
        ...
        for step in range(n_batches):
            ...
            epoch_sigma_reg += parts['sigma_reg']
            n_samples += len(batch)
            ...
            if (step + 1) % progress_interval == 0 and (step + 1) < n_batches:
                ...
                print(
                    ...
                    f"sig={run_sigma_reg/run_steps:.3f}) "
        ...
        print(f"  Train: loss={epoch_loss:.4f} "
              ...
              f"sig_reg={epoch_sigma_reg:.3f}) | {n_samples:,} samples")
        ...
        print(f"  L1:    lex={val['mean_l1_lex']:.0f}+/-{val['std_l1_lex']:.0f}  "
              f"res={val['mean_l1_residual']:+.1f}+/-{val['std_l1_residual']:.1f}")
        print(f"  Sigma: ffd={val['mean_sigma_ffd']:.3f} "
              f"gaze={val['mean_sigma_gaze']:.3f} "
              f"trt={val['mean_sigma_trt']:.3f} "
              f"(target={SIGMA_TARGET})")
        ...
        print(f"  Lex: bias={model.l1_bias.item():.1f} "
              f"freq={model.l1_freq_coef.item():.2f} "
              f"len={model.l1_len_coef.item():.2f} "
              f"surp={model.l1_surp_coef.item():.2f}")
        ...
        if is_best:
            ...
            torch.save({
                ...
                'cog_params': {
                    ...
                    'l1_bias': model.l1_bias.item(),
                    'l1_freq_coef': model.l1_freq_coef.item(),
                    'l1_len_coef': model.l1_len_coef.item(),
                    'l1_surp_coef': model.l1_surp_coef.item(),
                },
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "insanity_v2", f"geco_{model_short}"
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs", "train_insanity_v2_geco.log"
    )
