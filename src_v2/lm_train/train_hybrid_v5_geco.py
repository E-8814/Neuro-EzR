"""
Train the hybrid v5 Neural EZ Reader on GECO.

Same training recipe as train_hybrid_v4_geco.py (loss-scale normalization,
hinge skip prior, cosine LR schedule with warmup, patience-based early
stopping, L2 penalty on the skip residual logit) with the minimum changes
required by model_llama_hybrid_v5.NeuralEZReaderHybrid:

  - Model parameter attribute is `model.lm` (AutoModelForCausalLM) instead
    of `model.llama` (AutoModel).  The LM parameter group filter matches
    names starting with "lm.".
  - cog_name_prefixes: drops ezreader._M2_raw / ezreader._I_raw (tied in
    v5), adds ezreader._M2I_raw. Adds l1_pred_coef (alpha3).
  - print_cog_params / logging includes alpha3 (l1_pred_coef) and reports
    the tied M2=I value once.
  - Checkpoint dict saves l1_pred_coef.

The model's forward() signature is unchanged: (word_lists, frequencies,
word_lengths). Surprisal is computed inside the model using lm_head.
"""
...
from model_llama_hybrid_v5 import NeuralEZReaderHybrid
...
LAMBDA_DELTA = 5.0
LAMBDA_PRIOR = 20.0
LAMBDA_SKIP_RESIDUAL = 0.001   # L2 penalty on residual_skip_logit magnitude
SKIP_MIN = 0.35
SKIP_MAX = 0.55
DELTA_MIN = 0.10
DELTA_MAX = 0.50
...
def print_cog_params(model):
    ...
    print(f"    l1_base_offset  (alpha1)     = {model.l1_base_offset.item():7.2f} ms  (Reichle: 104)")
    print(f"    l1_freq_coef    (alpha2)     = {model.l1_freq_coef.item():7.2f}     (on normed log_freq; init -17)")
    print(f"    l1_pred_coef    (alpha3)     = {model.l1_pred_coef.item():7.2f} ms  (Reichle: 39; init 40)")
...
def print_sample_predictions(model, agg_data, device, subtlex, n_sentences=3, n_words=8):
    ...
    with torch.no_grad():
        for s_idx in range(min(n_sentences, len(agg_data))):
            ...
            print(f"  {'word':<14s} {'bL1':>5s} {'L1':>5s} {'L2':>5s} {'surp':>5s} {'p':>5s} {'ctx':>6s} | "
            ...
            for i in range(min(n_words, len(s.tokens))):
                ...
                err = ct - ht
                print(
                    f"  {s.tokens[i]:<14s} {bl1:5.0f} {l1:5.0f} {l2:5.0f} {surp:5.2f} {pred_p:5.3f} {ctx_val:+6.1f} | "
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v5/geco_tinyllama",
    log_path="../../logs/train_hybrid_v5_geco.log",
    seed=42,
    ...
    print("\n" + "=" * 100)
    print(f"Training HYBRID v5 model on GECO")
    ...
    print(f"  Loss: trt/SIGMA2 + ffd/SIGMA2 + gaze/SIGMA2 + bce + residual_reg")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f"  Cog: d={model.delta.item():.3f} "
              f"a1={model.l1_base_offset.item():.1f} "
              f"a2={model.l1_freq_coef.item():.2f} "
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "hybrid_v5", f"geco_{model_short}"
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs", "train_hybrid_v5_geco.log"
    )
