"""
Train the hybrid v4c Neural EZ Reader on GECO.

Same recipe as train_hybrid_v4_geco.py. v4c differs from v4 only
cosmetically:

  - M2 and I tied to single `_M2I_raw` (drops `_M2_raw` and `_I_raw`).
  - l1_neural_head renamed to ctx_head.
  - alpha1_reichle, alpha2_reichle properties exposed for logs.

Minimal adjustments here:

  - cog_name_prefixes: replaces ezreader._M2_raw / ezreader._I_raw with
    ezreader._M2I_raw.
  - print_cog_params reports alpha1_reichle, alpha2_reichle alongside
    the normalized parameterization.
  - Per-epoch log line uses a1R, a2R in Reichle units.
  - Save paths bumped from hybrid_v4 to hybrid_v4c.
"""
...
from model_llama_hybrid_v4c import NeuralEZReaderHybrid
...
def train(
    ...
    save_dir="../../checkpoints/hybrid_v4c/geco_tinyllama",
    log_path="../../logs/train_hybrid_v4c_geco.log",
    seed=42,
    ...
    print("\n" + "=" * 100)
    print(f"Training HYBRID v4c model on GECO  (v4 with cosmetic cleanup: M2=I tied, Reichle aliases, ctx_head)")
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "checkpoints", "hybrid_v4c",
        f"geco_{model_short}_seed{args.seed}",
    )
    log_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs",
        f"train_hybrid_v4c_geco_seed{args.seed}.log",
    )
